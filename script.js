...
async function doLogin() {
  ...
  if (acc.banned) {
    ...
    if (acc.banExpires && acc.banExpires > 0 && now >= acc.banExpires) {
      // Lift expired temp ban automatically
      await dbUpdateUser(u, { banned: false, banReason: null, banExpires: null, banType: null });
      acc.banned = false;
    } else {
      // Still banned — show deactivated screen
      btn.disabled=false; btn.textContent='Login';
      showBannedScreen(acc);
      return;
    }
  }
...
// ── Banned / Account Deactivated screen ───────────────────
function showBannedScreen(acc) {
  const isPerm = !acc.banExpires || acc.banExpires === 0;
  const expStr = isPerm ? 'Permanent' : (() => {
    const ms = acc.banExpires - Date.now();
    if (ms <= 0) return 'Expired';
    const d = Math.floor(ms/86400000);
    const h = Math.floor((ms%86400000)/3600000);
    const m = Math.floor((ms%3600000)/60000);
    return d>0 ? `${d}d ${h}h remaining` : h>0 ? `${h}h ${m}m remaining` : `${m}m remaining`;
  })();

  const reason = acc.banReason || 'No reason provided.';
  const bannedBy = acc.bannedBy || 'Administration';

  let overlay = document.getElementById('banned-screen');
  if (!overlay) {
    overlay = document.createElement('div');
    overlay.id = 'banned-screen';
    document.body.appendChild(overlay);
  }
  overlay.innerHTML = `
    <div class="banned-card">
      <div class="banned-icon">🚫</div>
      <div class="banned-title">Account Deactivated</div>
      <div class="banned-subtitle">${isPerm ? 'This account has been permanently banned.' : 'This account has been temporarily suspended.'}</div>
      ...
      <div class="banned-meta">
        <div class="banned-meta-row"><span class="banned-meta-key">Account</span><span class="banned-meta-val">${esc(acc.username)}</span></div>
        <div class="banned-meta-row"><span class="banned-meta-key">Issued by</span><span class="banned-meta-val">${esc(bannedBy)}</span></div>
        <div class="banned-meta-row"><span class="banned-meta-key">Duration</span><span class="banned-meta-val ${isPerm?'banned-perm':'banned-temp'}">${expStr}</span></div>
      </div>
      <div class="banned-footer">If you believe this is a mistake, contact an administrator.</div>
      <button class="banned-back-btn" onclick="document.getElementById('banned-screen').style.display='none'">← Back to Login</button>
    </div>`;
...
function enterApp() {
  ...
  // Show rules popup on every login
  setTimeout(showRulesOnLogin, 800);
}

// ── NAV ────────────────────────────────────────────────
function goTab(id) {
  document.querySelectorAll('.ntab').forEach((t,i)=>t.classList.toggle('on',['home','race','teams','items','inventory','shop','chat','lb','dm','society','trade','battlepass'][i]===id));
...
async function renderAPSAccounts() {
  ...
  el.innerHTML = accs.map(a => {
    ...
    const isPerm = isBanned && (!a.banExpires || a.banExpires === 0);
    const isTemp = isBanned && a.banExpires > 0;
    let banTag = '';
    if (isPerm) banTag = '<span class="aps-ban-tag perm">🚫 BANNED</span>';
    else if (isTemp) {
      const ms = a.banExpires - Date.now();
      const h = Math.max(0,Math.floor(ms/3600000));
      const m = Math.max(0,Math.floor((ms%3600000)/60000));
      banTag = ms > 0
        ? `<span class="aps-ban-tag temp">⏳ TEMP ${h}h ${m}m</span>`
        : '<span class="aps-ban-tag expired">✅ EXPIRED</span>';
    }
    return `
    <div class="aps-acc-row ${isBanned?'aps-acc-banned':''}">
      <div class="aps-acc-name">${esc(a.username)} <span style="color:var(--muted);font-size:.75rem">${a.muted?'🔇':''}</span> ${banTag}</div>
      ...
      <div class="aps-acc-acts">
        ...
        ${isBanned
          ...
          : `<button class="bsm take" onclick="openBanModal('${esca(a.username)}')">⏳ Temp Ban</button>
             <button class="bsm del"  onclick="openPermBanModal('${esca(a.username)}')">🚫 Perm Ban</button>`
        }
...
async function apsDel(u) { if(!confirm(`Permanently DELETE "${u}"? This cannot be undone.`))return; await dbDeleteUser(u); if(u===getU()){doLogout();return;} showToast(`Deleted ${u}`); renderAPSAccounts(); }

// ── Ban Modal ─────────────────────────────────────────────
function openBanModal(username) {
  _apsBanTarget = username;
  _apsBanPerm = false;
  const m = document.getElementById('aps-ban-modal');
  document.getElementById('aps-ban-modal-title').textContent = `⏳ Temp Ban — ${username}`;
  document.getElementById('aps-ban-duration-row').style.display = 'flex';
  document.getElementById('aps-ban-reason').value = '';
  document.getElementById('aps-ban-duration').value = '24';
  m.style.display = 'flex';
}

function openPermBanModal(username) {
  _apsBanTarget = username;
  _apsBanPerm = true;
  const m = document.getElementById('aps-ban-modal');
  document.getElementById('aps-ban-modal-title').textContent = `🚫 Permanent Ban — ${username}`;
  document.getElementById('aps-ban-duration-row').style.display = 'none';
  document.getElementById('aps-ban-reason').value = '';
  m.style.display = 'flex';
}

function closeBanModal() {
  document.getElementById('aps-ban-modal').style.display = 'none';
  _apsBanTarget = null;
}

let _apsBanTarget = null, _apsBanPerm = false;

async function confirmBan() {
  const u = _apsBanTarget;
  if (!u) return;
  const reason = document.getElementById('aps-ban-reason').value.trim() || 'No reason provided.';
  const hours = _apsBanPerm ? 0 : (parseInt(document.getElementById('aps-ban-duration').value) || 24);
  const banExpires = _apsBanPerm ? 0 : (Date.now() + hours * 3600000);

  await dbUpdateUser(u, {
    banned: true,
    banReason: reason,
    banType: _apsBanPerm ? 'permanent' : 'temporary',
  ...
  closeBanModal();
  showToast(_apsBanPerm ? `🚫 ${u} permanently banned` : `⏳ ${u} temp banned for ${hours}h`);
  renderAPSAccounts();
}

async function apsUnban(u) {
  if (!confirm(`Remove ban from "${u}"?`)) return;
  await dbUpdateUser(u, { banned: false, banReason: null, banExpires: null, banType: null, bannedBy: null });
...
function mgrSwitchTab(tab) {
  ...
  document.getElementById('mgr-sec-rules').style.display = tab === 'rules' ? 'block' : 'none';
  ...
  if (tab === 'rules') renderMgrRulesList();
}
...
// Load rules from Firebase / localStorage
async function loadRules() {
  if (FB_READY) {
    try {
      const snap = await db.collection('rules').orderBy('order', 'asc').get();
      rulesCache = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    } catch(e) {
      rulesCache = JSON.parse(localStorage.getItem('lt_rules') || '[]');
    }
  } else {
    rulesCache = JSON.parse(localStorage.getItem('lt_rules') || '[]');
  }
...
// Manager: render rules list
function renderMgrRulesList() {
  const el = document.getElementById('mgr-rules-list');
  if (!el) return;
  if (!rulesCache.length) {
    el.innerHTML = '<div class="empty">No rules yet. Click + New Rule.</div>';
    return;
  }
  el.innerHTML = rulesCache.map(r => {
    const sevColor = r.severity === 'strict' ? '#ff4444' : r.severity === 'warn' ? '#ffaa00' : '#4ddbff';
    const sevLabel = r.severity === 'strict' ? '🚫 Strict' : r.severity === 'warn' ? '⚠️ Warning' : 'ℹ️ Info';
    return `
    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;padding:12px 14px;background:rgba(255,255,255,.03);border:1px solid rgba(180,120,0,.15);border-radius:8px;margin-bottom:8px">
      <div style="flex:1;min-width:0">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">
          <span style="font-family:'Bebas Neue',cursive;font-size:1.1rem;color:#ffcc44">Rule ${esc(r.num||'?')}</span>
          <span style="font-size:.75rem;padding:2px 8px;border-radius:4px;background:${sevColor}22;color:${sevColor};border:1px solid ${sevColor}55">${sevLabel}</span>
        </div>
        <div style="font-weight:700;font-size:.9rem;color:var(--text);margin-bottom:2px">${esc(r.title)}</div>
        <div style="font-size:.78rem;color:var(--muted);line-height:1.4">${esc(r.desc||'')}</div>
      </div>
      <div style="display:flex;flex-direction:column;gap:5px;flex-shrink:0">
        <button class="bsm edit" onclick="mgrEditRule('${esca(r.id)}')">✏ Edit</button>
        <button class="bsm del" onclick="mgrDeleteRule('${esca(r.id)}')">🗑 Del</button>
      </div>
    </div>`;
  }).join('');
}

// Show/populate rule form
function mgrShowRuleForm(entry) {
  const form = document.getElementById('mgr-rule-form');
  form.style.display = 'block';
  document.getElementById('mgr-rule-edit-id').value = entry ? entry.id : '';
  document.getElementById('mgr-rule-num').value = entry ? (entry.num || '') : (rulesCache.length + 1);
  document.getElementById('mgr-rule-title').value = entry ? entry.title : '';
  document.getElementById('mgr-rule-desc').value = entry ? (entry.desc || '') : '';
  document.getElementById('mgr-rule-severity').value = entry ? (entry.severity || 'info') : 'info';
  document.getElementById('mgr-rule-form-title').textContent = entry ? 'Edit Rule' : 'New Rule';
  document.getElementById('mgr-rule-num').focus();
  form.scrollIntoView({ behavior: 'smooth', block: 'center' });
}

function mgrEditRule(id) {
  const entry = rulesCache.find(r => r.id === id);
  if (entry) mgrShowRuleForm(entry);
}

// Save new/edited rule
async function mgrSaveRule() {
  const editId  = document.getElementById('mgr-rule-edit-id').value;
  const num     = document.getElementById('mgr-rule-num').value.trim();
  const title   = document.getElementById('mgr-rule-title').value.trim();
  const desc    = document.getElementById('mgr-rule-desc').value.trim();
  const severity = document.getElementById('mgr-rule-severity').value;

  if (!title) { showToast('Rule title is required.'); return; }

  const payload = { num, title, desc, severity, order: parseInt(num) || rulesCache.length + 1, updatedAt: Date.now() };

  try {
    if (FB_READY) {
      if (editId) {
        await db.collection('rules').doc(editId).update(payload);
      } else {
        await db.collection('rules').add({ ...payload, createdAt: Date.now() });
      }
    } else {
      let list = JSON.parse(localStorage.getItem('lt_rules') || '[]');
      if (editId) {
        const idx = list.findIndex(r => r.id === editId);
        if (idx >= 0) list[idx] = { ...list[idx], ...payload };
      } else {
        list.push({ id: 'r' + Date.now(), ...payload, createdAt: Date.now() });
      }
      localStorage.setItem('lt_rules', JSON.stringify(list));
    }
    await loadRules();
    renderMgrRulesList();
    document.getElementById('mgr-rule-form').style.display = 'none';
    showToast(editId ? '📜 Rule updated!' : '📜 Rule created!');
  } catch(e) {
    console.error('mgrSaveRule:', e);
    showToast('Error saving rule: ' + e.message);
  }
}

// Delete rule
async function mgrDeleteRule(id) {
  ...
  try {
    ...
    showToast('Error deleting rule: ' + e.message);
  }
}

// ── Show rules popup on login ──
async function showRulesOnLogin() {
  await loadRules();
  ...
  if (!rulesCache.length) {
    // No rules set → skip popup
    return;
  }

  const sevColor = { info: '#4ddbff', warn: '#ffaa00', strict: '#ff4444' };
  const sevIcon  = { info: 'ℹ️', warn: '⚠️', strict: '🚫' };

  list.innerHTML = rulesCache.map(r => {
    const col = sevColor[r.severity] || '#4ddbff';
    const icon = sevIcon[r.severity] || 'ℹ️';
    return `
    <div style="display:flex;gap:14px;padding:14px 16px;background:rgba(255,255,255,.04);border:1px solid ${col}33;border-left:3px solid ${col};border-radius:8px">
      <div style="font-size:1.3rem;flex-shrink:0;padding-top:1px">${icon}</div>
      <div style="flex:1">
        <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:4px">
          <span style="font-family:'Bebas Neue',cursive;font-size:1rem;letter-spacing:2px;color:${col}">Rule ${esc(r.num||'')}</span>
          <span style="font-weight:700;font-size:.95rem;color:var(--text)">${esc(r.title)}</span>
        </div>
        ${r.desc ? `<div style="font-size:.85rem;color:var(--muted);line-height:1.5">${esc(r.desc)}</div>` : ''}
      </div>
    </div>`;
  }).join('');

  overlay.style.display = 'flex';
  // Prevent scroll on body while rules are shown
  document.body.style.overflow = 'hidden';
}

// Accept rules and close overlay
function acceptRules() {
  const overlay = document.getElementById('rules-overlay');
  if (overlay) overlay.style.display = 'none';
  document.body.style.overflow = '';
  showToast('✅ Rules accepted! Welcome to LiquidType.');
}
