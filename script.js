...
// â”€â”€ DATA LAYER â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ IMAGE UPLOAD via ImgBB â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ CHEAT DETECTION â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ AUTO-DELETE OLD MESSAGES (>24h) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ TEAMS SYSTEM â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
let teamCache = null;
let teamChatCache = [];
...
// Team chat functions
// Token incremented every time the listener changes â€” lets async callbacks
...
function startTeamChatListener(teamId) {
  ...
  if (FB_READY) {
    ...
    teamChatUnsub = db.collection('team_messages')
      ...
      .onSnapshot(
        snap => {
          if (myToken !== teamChatToken) return; // stale â€” a newer listener took over
...
// â”€â”€ STREAK HELPER â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ FEUDALISM SYSTEM â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function updateKingdomUI() {
  ...
  el.innerHTML = `King: <span style="color:${RANK_COLORS['King']}">${FS.king || 'Election'}</span> | Treasury: ðŸ’§${FS.treasury}<br>Revolt: <span style="color:${pct >= 100 ? 'var(--ok)' : 'var(--bad)'}">${pct}%</span> toward 80% threshold`;
...
async function arrestUser(username, mins) {
  ...
  if (username === getU()) {
    ...
    showToast(`ðŸš¨ ARRESTED! You are in jail for ${mins}m for tax evasion.`);
  }
...
async function renderSocietyTab() {
  ...
  if (FS.proclamation) {
    proclamationHtml = `
      <div style="grid-column:1/-1;background:linear-gradient(135deg,rgba(255,215,0,.08),rgba(120,80,0,.12));border:1px solid rgba(255,215,0,.35);border-radius:12px;padding:16px 20px;text-align:center">
        <div style="font-family:'Bebas Neue',cursive;font-size:.75rem;letter-spacing:3px;color:rgba(255,215,0,.6);margin-bottom:6px">ðŸ“œ ROYAL PROCLAMATION</div>
        <div style="font-size:.95rem;color:#ffd700;font-style:italic">"${esc(FS.proclamation)}"</div>
        <div style="font-size:.72rem;color:rgba(255,215,0,.45);margin-top:6px">â€” ${esc(FS.king || 'The Crown')}</div>
      </div>`;
  }
  ...
  if (isKing && FB_READY) {
    ...
    petitionsHtml = `
      <div class="card-panel" style="grid-column:1/-1;border-color:#ffd700;background:rgba(255,215,0,.02)">
        <div class="h-card-title" style="color:#ffd700">ðŸ“œ Royal Petitions (${petitions.length})</div>
        <div>
          ${petitions.map(p => `
  ...
  const kingPanel = isKing ? `
    <div class="card-panel" style="grid-column:1/-1;border-color:#ffd700;background:linear-gradient(135deg,rgba(255,215,0,.04),rgba(120,60,0,.06))">
      <div class="h-card-title" style="color:#ffd700;font-size:1.2rem">ðŸ‘‘ Royal Command Panel</div>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;margin-top:10px">
        ...
        <div style="background:rgba(255,40,40,.06);border:1px solid rgba(255,40,40,.25);border-radius:10px;padding:14px">
          <div style="font-family:'Bebas Neue',cursive;font-size:.9rem;letter-spacing:2px;color:#ff6666;margin-bottom:10px">â›“ Dungeon</div>
          <input id="king-dungeon-user" type="text" placeholder="Usernameâ€¦" style="width:100%;padding:7px 9px;background:rgba(0,0,0,.4);border:1px solid rgba(255,40,40,.3);border-radius:6px;color:var(--text);font-family:'Rajdhani',sans-serif;font-size:.88rem;outline:none;margin-bottom:6px">
          ...
          <button class="rbtn" style="width:100%;background:rgba(180,0,0,.5);border:1px solid #cc2222;font-size:.82rem;padding:8px" onclick="kingThrowInDungeon()">â›“ Throw in Dungeon</button>
        </div>

        <div style="background:rgba(100,0,200,.06);border:1px solid rgba(100,0,200,.25);border-radius:10px;padding:14px">
          <div style="font-family:'Bebas Neue',cursive;font-size:.9rem;letter-spacing:2px;color:#aa66ff;margin-bottom:10px">ðŸš« Exile</div>
          <input id="king-exile-user" type="text" placeholder="Usernameâ€¦" style="width:100%;padding:7px 9px;background:rgba(0,0,0,.4);border:1px solid rgba(100,0,200,.3);border-radius:6px;color:var(--text);font-family:'Rajdhani',sans-serif;font-size:.88rem;outline:none;margin-bottom:6px">
          ...
          <button class="rbtn" style="width:100%;background:rgba(80,0,160,.5);border:1px solid #8833cc;font-size:.82rem;padding:8px" onclick="kingExilePlayer()">ðŸš« Exile from Kingdom</button>
        </div>

        <div style="background:rgba(255,170,0,.05);border:1px solid rgba(255,170,0,.25);border-radius:10px;padding:14px">
          <div style="font-family:'Bebas Neue',cursive;font-size:.9rem;letter-spacing:2px;color:#ffbb44;margin-bottom:10px">ðŸ’° Royal Tax</div>
          ...
          <button class="rbtn" style="width:100%;background:rgba(160,100,0,.5);border:1px solid #cc8800;font-size:.82rem;padding:8px" onclick="kingCollectTax()">ðŸ’° Collect Tax</button>
        </div>

        <div style="background:rgba(255,80,0,.05);border:1px solid rgba(255,80,0,.25);border-radius:10px;padding:14px">
          <div style="font-family:'Bebas Neue',cursive;font-size:.9rem;letter-spacing:2px;color:#ff8844;margin-bottom:10px">ðŸŽ¯ Bounty</div>
          <input id="king-bounty-user" type="text" placeholder="Usernameâ€¦" style="width:100%;padding:7px 9px;background:rgba(0,0,0,.4);border:1px solid rgba(255,80,0,.3);border-radius:6px;color:var(--text);font-family:'Rajdhani',sans-serif;font-size:.88rem;outline:none;margin-bottom:6px">
          <input id="king-bounty-amt" type="number" placeholder="Bounty ðŸ’§" value="500" min="1" style="width:100%;padding:7px 9px;background:rgba(0,0,0,.4);border:1px solid rgba(255,80,0,.3);border-radius:6px;color:var(--text);font-family:'Rajdhani',sans-serif;font-size:.88rem;outline:none;margin-bottom:8px">
          <button class="rbtn" style="width:100%;background:rgba(180,60,0,.5);border:1px solid #cc5500;font-size:.82rem;padding:8px" onclick="kingPlaceBounty()">ðŸŽ¯ Place Bounty</button>
        </div>

        <div style="background:rgba(0,170,255,.05);border:1px solid rgba(0,170,255,.25);border-radius:10px;padding:14px">
          <div style="font-family:'Bebas Neue',cursive;font-size:.9rem;letter-spacing:2px;color:#44aaff;margin-bottom:10px">âšœï¸ Appoint Rank</div>
          <input id="king-appoint-user" type="text" placeholder="Usernameâ€¦" style="width:100%;padding:7px 9px;background:rgba(0,0,0,.4);border:1px solid rgba(0,170,255,.3);border-radius:6px;color:var(--text);font-family:'Rajdhani',sans-serif;font-size:.88rem;outline:none;margin-bottom:6px">
          <select id="king-appoint-rank" style="width:100%;padding:7px 9px;background:rgba(0,0,0,.4);border:1px solid rgba(0,170,255,.3);border-radius:6px;color:var(--text);font-family:'Rajdhani',sans-serif;font-size:.88rem;outline:none;margin-bottom:8px">
            <option value="Clear">ðŸ”„ Clear (Auto)</option><option value="Noble">ðŸ’Ž Noble</option>
            <option value="Knight">âš”ï¸ Knight</option><option value="Serf">ðŸ“œ Serf</option>
          </select>
          <button class="rbtn" style="width:100%;background:rgba(0,80,180,.5);border:1px solid #0066cc;font-size:.82rem;padding:8px" onclick="kingAppointRank()">âšœï¸ Appoint</button>
        </div>

        <div style="background:rgba(255,215,0,.04);border:1px solid rgba(255,215,0,.2);border-radius:10px;padding:14px">
          <div style="font-family:'Bebas Neue',cursive;font-size:.9rem;letter-spacing:2px;color:#ffd700;margin-bottom:10px">ðŸ“£ Proclamation</div>
          <textarea id="king-proclamation" rows="3" placeholder="Declare something to the kingdomâ€¦" maxlength="200" style="width:100%;padding:7px 9px;background:rgba(0,0,0,.4);border:1px solid rgba(255,215,0,.3);border-radius:6px;color:var(--text);font-family:'Rajdhani',sans-serif;font-size:.88rem;outline:none;resize:none;margin-bottom:8px">${esc(FS.proclamation||'')}</textarea>
          <button class="rbtn" style="width:100%;background:rgba(120,80,0,.5);border:1px solid #cc9900;font-size:.82rem;padding:8px" onclick="kingIssueProclamation()">ðŸ“£ Issue Proclamation</button>
        </div>

        <div style="background:rgba(255,215,0,.03);border:1px solid rgba(255,215,0,.15);border-radius:10px;padding:14px">
          <div style="font-family:'Bebas Neue',cursive;font-size:.9rem;letter-spacing:2px;color:#ffd700;margin-bottom:10px">ðŸ“Š Rank Thresholds</div>
          <div style="font-size:.72rem;color:var(--muted);margin-bottom:4px">Noble (ðŸ’§)</div>
          ...
          <div style="font-size:.72rem;color:var(--muted);margin-bottom:4px">Knight (ðŸ’§)</div>
          ...
          <button class="rbtn" style="width:100%;background:rgba(100,70,0,.5);border:1px solid #cc9900;font-size:.82rem;padding:8px" onclick="saveThresholdsFromSociety()">ðŸ’¾ Save</button>
        </div>

        <div style="background:rgba(0,200,100,.04);border:1px solid rgba(0,200,100,.2);border-radius:10px;padding:14px">
          <div style="font-family:'Bebas Neue',cursive;font-size:.9rem;letter-spacing:2px;color:#00dd77;margin-bottom:10px">ðŸ¦ Treasury: ðŸ’§${FS.treasury}</div>
          <button class="rbtn" style="width:100%;background:rgba(0,100,50,.4);border:1px solid #00aa55;font-size:.82rem;padding:8px;margin-bottom:6px" onclick="buyRoyalBuff('Banquet',1.2,5000,3600000)">ðŸ– Banquet 1.2Ã— (5k)</button>
          <button class="rbtn" style="width:100%;background:rgba(0,80,40,.4);border:1px solid #00aa55;font-size:.82rem;padding:8px;margin-bottom:6px" onclick="buyRoyalBuff('Golden Era',1.5,15000,3600000)">âœ¨ Golden Era 1.5Ã— (15k)</button>
          <input id="king-emb-amt" type="number" placeholder="Embezzle ðŸ’§â€¦" min="1" style="width:100%;padding:7px 9px;background:rgba(0,0,0,.4);border:1px solid rgba(255,50,50,.25);border-radius:6px;color:var(--text);font-family:'Rajdhani',sans-serif;font-size:.88rem;outline:none;margin-bottom:6px">
          <button class="rbtn" style="width:100%;background:rgba(120,0,0,.4);border:1px solid #aa2222;font-size:.82rem;padding:8px" onclick="kingEmbezzle()">ðŸ¤« Pocket Funds</button>
        </div>
  ...
  const hierarchyHtml = Object.entries(hierarchy).map(([rank, users]) => {
    ...
    const icon = rank==='King'?'ðŸ‘‘':rank==='Noble'?'ðŸ’Ž':rank==='Knight'?'âš”ï¸':'ðŸ“œ';
    return `
      <div style="flex:1;min-width:140px">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:5px">
          ${users.map(u => {
            ...
            return `<div style="padding:4px 10px;border-radius:12px;background:rgba(255,255,255,.05);border:1px solid ${isBountied?'#ff8800':'rgba(255,255,255,.08)'};font-size:.82rem;cursor:pointer;color:${isBountied?'#ff8800':'inherit'}" onclick="openProfile('${esca(u.username)}')" title="${esc(u.username)}${isBountied?' ðŸŽ¯ Bounty: '+u.bounty+'ðŸ’§':''}">${esc(u.username)}${isBountied?'ðŸŽ¯':''}</div>`;
  ...
  const dungeonHtml = jailed.map(u => `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:rgba(255,40,40,.06);border:1px solid rgba(255,40,40,.2);border-radius:8px;margin-bottom:6px">
      ...
      ${isKing?`<button class="bsm give" style="font-size:.75rem" onclick="pardonUser('${esca(u.username)}')">âœ… Pardon</button>`:''}
  ...
  const exileHtml = exiled.map(u => `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:rgba(100,0,200,.06);border:1px solid rgba(100,0,200,.2);border-radius:8px;margin-bottom:6px">
      ...
      ${isKing?`<button class="bsm give" style="font-size:.75rem" onclick="kingLiftExile('${esca(u.username)}')">ðŸ”“ Lift Exile</button>`:''}
  ...
  const bountyHtml = bounties.map(u => `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:rgba(255,80,0,.06);border:1px solid rgba(255,80,0,.2);border-radius:8px;margin-bottom:6px">
      ...
      <div style="font-weight:700;color:#ff8844">ðŸŽ¯ ${u.bounty}ðŸ’§</div>
      ${isKing?`<button class="bsm del" style="font-size:.75rem" onclick="kingClearBounty('${esca(u.username)}')">âœ•</button>`:''}
  ...
  container.innerHTML = `
    <div class="soc-grid">
      ...
      <div class="card-panel hier-main-card" style="grid-column:1/-1">
        <div class="h-card-title">ðŸ° Kingdom Hierarchy</div>
      ...
      <div class="card-panel" style="border-color:#ff4444">
        <div class="h-card-title" style="color:#ff4444">â›“ The Dungeon</div>
        ${dungeonHtml}
      </div>

      <div class="card-panel" style="border-color:#8833cc">
        <div class="h-card-title" style="color:#aa66ff">ðŸš« Exiled</div>
        ${exileHtml}
      </div>

      <div class="card-panel" style="border-color:#ff6600">
        <div class="h-card-title" style="color:#ff8844">ðŸŽ¯ Bounty Board</div>
        ${bountyHtml}
      </div>

      <div class="card-panel">
        <div class="h-card-title">ðŸ’° Royal Treasury</div>
        <div style="text-align:center;padding:15px">
          <div style="font-size:2.5rem;color:#ffd700">ðŸ’§ ${FS.treasury}</div>
      ...
      <div class="card-panel" style="text-align:center;border-color:#ffd700">
        <div class="h-card-title">ðŸ“œ Petition the Crown</div>
        ...
        <button class="rbtn" onclick="castRevoltVote()" style="width:100%;margin-top:8px;background:#300000;border:1px solid #880000;color:#ff4444">âš”ï¸ Vote to Revolt</button>
      </div>`:''}
    </div>
  `;
}
...
function checkJail() {
  if (UC && UC.jailUntil > Date.now()) {
    ...
    showToast(`ðŸš« You are in JAIL! ${remaining}m remaining.`);
    return true;
  }
  return false;
}
...
function openSlaveryMinigame() {
  ...
  overlay.innerHTML = `<div class="modal"><div class="mttl">WEEKLY LABOR</div><div class="msub">Your master demands tribute. Mine 50 ðŸ’§.</div><div style="text-align:center;padding:20px;"><button class="rbtn" id="work-btn" onclick="doSlaveWork()">â›ï¸ MINE (0/10)</button></div></div>`;
...
async function doSlaveWork() {
  ...
  btn.textContent = `â›ï¸ MINE (${window._workCount}/10)`;
  if (window._workCount >= 10) {
    ...
    showToast("Work finished! You earned 50 ðŸ’§.");
  }
}
...
async function openKingdomManager() {
  ...
  overlay.className = 'moverlay on';
  overlay.innerHTML = `<div class="modal"><div class="mttl">ðŸ‘‘ ROYAL MANAGEMENT</div><div class="msub">Treasury: ðŸ’§${FS.treasury}</div>
...
async function embezzleTreasury() {
  ...
  if (amt > FS.treasury) { showToast(`Treasury only has ðŸ’§${FS.treasury}`); return; }
  ...
  await logRoyalAction(`The King pocketed ðŸ’§${amt} for personal use.`);
...
// â”€â”€ REBIRTH SYSTEM â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function openRebirthModal() {
  ...
  if (!overlay) return;
  document.getElementById('rebirth-cost-display').textContent = REBIRTH_COST.toLocaleString() + ' ðŸ§¢';
  document.getElementById('rebirth-balance-display').textContent = (UC.coins || 0).toLocaleString() + ' ðŸ§¢';
...
async function doRebirth() {
  ...
  if ((UC.coins || 0) < REBIRTH_COST) {
    errEl.textContent = `Not enough bottlecaps! You need ${REBIRTH_COST.toLocaleString()} ðŸ§¢.`;
    return;
  }

  if (!confirm(`âš  Final confirmation: You will lose ALL your bottlecaps and themes. You'll gain 1 Plasma. Continue?`)) return;
  ...
  try {
    ...
    // Wipe coins, themes â€” keep only default theme
    UC.coins       = 0;
    UC.themes      = ['default'];
    ...
    closeRebirthModal();
    showToast(`âš— Rebirth ${newRebirths} complete! You now have ${newPlasma} Plasma.`);
  } catch(e) {
    console.error('Rebirth failed:', e);
    errEl.textContent = 'Rebirth failed â€” try again.';
    if (btn) { btn.disabled = false; btn.textContent = 'âš— REBIRTH'; }
  }
}
// â”€â”€ END REBIRTH SYSTEM â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

// â”€â”€ PLASMA SHOP â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
/*
  Plasma Items fall into 2 categories:
    CONSUMABLES â€” stored as quantities in UC.plasmaConsumables = { itemId: qty }
                  Show up in the Inventory tab with a USE button.
    PERKS       â€” stored as a set in UC.plasmaPerks = [id, id, ...]
...
const PLASMA_CONSUMABLES = [
  {
    ...
    icon:  'ðŸ“¦',
    cost:  1,
    desc:  'Open for a random coin reward between 100 â€“ 5,000 ðŸ§¢. Could be your lucky day.',
    rarity:'Common',
    rarityColor: '#aaaaaa',
  },
  {
  ...
  },
  {
    ...
    icon:  'ðŸ’¥',
    cost:  3,
    desc:  'Activate to earn 2Ã— coins from all races for 10 minutes.',
    rarity:'Rare',
    rarityColor: '#44aaff',
  },
  {
    ...
    icon:  'ðŸŒ€',
    cost:  5,
    desc:  'A mysterious relic. Use to trigger a random powerful effect â€” or something weird.',
    rarity:'Epic',
    rarityColor: '#cc44ff',
  },
];
  ...
  grid.innerHTML = PLASMA_PERKS.map(perk => {
    ...
    return `
      <div class="ps-card ${owned ? 'ps-card-owned' : ''}">
        ...
        ${owned
          ? `<div class="ps-owned-badge">âœ… Owned â€” Active</div>`
          : `<button class="ps-buy-btn" onclick="buyPlasmaPerk('${perk.id}')" ${!canAfford ? 'disabled' : ''}>âš— ${cost} Plasma</button>`
        }
      </div>`;
  }).join('');
}
...
function renderPSOwned() {
    ...
    .map(item => `
      <div class="ps-owned-row">
        ...
        <div class="ps-owned-info">
          <div class="ps-owned-name">${item.name} <span style="color:#aa44ff">Ã—${inv[item.id]}</span></div>
  ...
  const perkRows = PLASMA_PERKS
    ...
    .map(p => `
      <div class="ps-owned-row">
        ...
        <div style="color:#00e676;font-size:.8rem;font-weight:700;white-space:nowrap">âœ… Active</div>
      </div>`).join('');
  ...
  list.innerHTML = `
    ${consumableRows ? `<div style="font-family:'Bebas Neue',cursive;letter-spacing:2px;color:#cc88ff;margin-bottom:10px;font-size:1.1rem">ðŸŽ² Consumables</div>${consumableRows}` : ''}
    ${perkRows ? `<div style="font-family:'Bebas Neue',cursive;letter-spacing:2px;color:#cc88ff;margin:18px 0 10px;font-size:1.1rem">âš¡ Permanent Perks</div>${perkRows}` : ''}
  `;
}
...
async function buyPlasmaConsumable(itemId) {
  ...
  renderPSConsumables();
  showToast(`âœ… ${item.icon} ${item.name} added to your inventory!`);
}
...
async function buyPlasmaPerk(perkId) {
  ...
  document.getElementById('ps-balance').textContent = UC.plasma;
  renderPSPerks();
  showToast(`âœ… ${perk.icon} ${perk.name} is now permanently active!`);
}
...
async function usePlasmaItem(itemId) {
  ...
  if (itemId === 'lucky_box') {
    // â”€â”€ Lucky Box: 100â€“5000 coins, weighted toward lower â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    const roll = Math.random();
    let reward;
    if      (roll < 0.50) reward = Math.floor(Math.random() * 400) + 100;   // 50%: 100â€“500
    else if (roll < 0.80) reward = Math.floor(Math.random() * 500) + 500;   // 30%: 500â€“1000
    else if (roll < 0.95) reward = Math.floor(Math.random() * 2000) + 1000; // 15%: 1000â€“3000
    else                  reward = Math.floor(Math.random() * 2000) + 3000; //  5%: 3000â€“5000
    ...
    document.getElementById('lb-icon').textContent    = isJackpot ? 'ðŸ’Ž' : 'ðŸ“¦';
    ...
    document.getElementById('lb-result-text').innerHTML =
      `You opened a Lucky Box and found<br><span style="font-family:'Bebas Neue',cursive;font-size:2.5rem;color:${isJackpot?'#ffcc00':'#00e676'}">+${reward.toLocaleString()} ðŸ§¢</span>`;
    closePlasmaShop();
    ...
    showToast('ðŸŸ¢ Plasma Lure active! Next 10 DePoule pets are guaranteed GREEN.');
    closePlasmaShop();

  } else if (itemId === 'coin_surge') {
    _coinSurgeUntil = Date.now() + 10 * 60 * 1000; // 10 minutes
    await dbUpdateUser(getU(), { plasmaConsumables: inv });
    showToast('ðŸ’¥ Coin Surge active! 2Ã— race coins for 10 minutes!');
    closePlasmaShop();
    ...
    const effects = [
      async () => {
        ...
        await dbUpdateUser(getU(),{coins:UC.coins,plasmaConsumables:inv});
        refreshCoins();
        return `The Void grants you <b style="color:#ffcc00">+${r.toLocaleString()} ðŸ§¢</b>. The void is generous today.`;
      },
      async () => {
        const bonus = 1;
        ...
        refreshCoins();
        return `The Void echoes back. You gained <b style="color:#aa44ff">+${bonus} âš— Plasma</b>.`;
      },
      async () => {
      ...
      async () => {
        ...
        await dbUpdateUser(getU(),{plasmaConsumables:inv});
        return `Void energy surges through the server. <b style="color:#ff8800">2Ã— race coins for 5 minutes</b>.`;
      },
      async () => {
        ...
        await dbUpdateUser(getU(),{coins:UC.coins,plasmaConsumables:inv});
        refreshCoins();
        return `The Void takes. You lose <b style="color:#f44">âˆ’${loss} ðŸ§¢</b>. It happens.`;
      },
    ];
    const pick = effects[Math.floor(Math.random()*effects.length)];
    const msg = await pick();
    saved = false; // already saved inside each effect
    document.getElementById('lb-icon').textContent    = 'ðŸŒ€';
...
// â”€â”€ END PLASMA SHOP â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ SOLO RACE ENGINE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function soloFinished() {
  ...
  RS.bots.forEach((b,i)=>{
    if(!b.finished){
      // Bot hadn't finished yet â€” it finishes AFTER player
      b.finished=true;
...
function showResult(place,coins,wpm,acc,elapsed,bpXp) {
  ...
  document.getElementById('r-coins').textContent='+'+coins+' ðŸ§¢';
  ...
  if (xpWrap) {
    if (bpXp && bpXp.xp > 0) {
      ...
      xpWrap.innerHTML = `âš” +${bpXp.xp} BP XP${bpXp.leveledUp ? ` <span style="color:#fff;background:linear-gradient(90deg,#ff8800,#ffcc00);padding:1px 8px;border-radius:8px;font-size:.8rem;margin-left:4px">LEVEL UP! â†’ Lv.${bpXp.newLevel}</span>` : ''}`;
...
function resetRace() {
  ...
  document.getElementById('ptext').innerHTML='<span style="color:var(--muted);font-size:.88rem">Press Start Race to beginâ€¦</span>';
  document.getElementById('result-box').style.display='none';
...
// â”€â”€ LIVE RACE ENGINE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function startLiveSearch() {
  ...
  document.getElementById('btn-solo').style.display='none';
  document.getElementById('btn-live').style.display='none';
  document.getElementById('search-status').textContent='Searching for opponentsâ€¦';
...
function handleLobbySnap(lobby) {
  if(lobby.status==='racing'&&!liveStarted&&liveRS.searching) {
    ...
    document.getElementById('search-matched').textContent='ðŸŽ® '+liveRS.opUser+' joined â€” race starting!';
    ...
    if(lobby[opRole+'Done']&&!RS.finished) {
      // Opponent beat us â€” end our race
...
// â”€â”€ SHOP / THEMES â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const THEMES=[
  ...
  {id:'holographic',name:'Holographic',desc:'Shifting rainbow holo. âœ¨',price:400,prev:'prev-holographic'},
...
function renderShop() {
  ...
  THEMES.forEach(t=>{
    ...
    const qtyBadge=qty>0?`<div style="position:absolute;top:6px;right:6px;background:rgba(0,0,0,.75);border:1px solid rgba(255,255,255,.2);border-radius:10px;padding:1px 7px;font-size:.7rem;font-weight:700;color:#fff">Ã—${qty}</div>`:'';
    let act='';
    if(t.price===0){
      if(active) act=`<div class="badge-on">Active</div><button class="towned">âœ“ Equipped</button>`;
      ...
      const priceTag=`<div class="tprice">${saved>0?'<s style="color:var(--muted);font-size:.75rem">ðŸ§¢ '+t.price+'</s> ':''}ðŸ§¢ ${dp}${saved>0?' <span style="color:#00e676;font-size:.72rem">-'+Math.round(saved/t.price*100)+'%</span>':''}</div>`;
      ...
      else if(owned||activeMods.has('litematica')) act=priceTag+`<button class="tequip" onclick="equipTheme('${t.id}')">Equip${activeMods.has('litematica')&&!owned?' ðŸŽ¨':''}</button><button class="tbuy" style="margin-top:4px" onclick="buyTheme('${t.id}',${t.price})" ${!canAfford?'disabled':''}>Buy Again</button>`;
...
async function buyTheme(id,price){
  ...
  showToast('Theme unlocked! ðŸŽ‰'+(savedAmt>0?' (saved '+savedAmt+' ðŸ§¢)':''));
}
...
async function renderItemsShop() {
  ...
  grid.innerHTML = items.map(item => {
    ...
    let button = '';
    if (owned) {
      button = '<div class="item-owned">âœ“ OWNED</div>';
    } else if (outOfStock) {
      button = '<div class="item-sold-out">SOLD OUT</div>';
    } else if (item.unique && owned) {
      button = '<div class="item-owned">âœ“ OWNED</div>';
    } else {
      button = `<button class="item-buy-btn" onclick="buyItem('${esca(item.id)}')" ${!canBuy ? 'disabled' : ''}>Buy for ${item.price} ðŸ’§</button>`;
    }
    
    return `
      <div class="item-card">
        <div class="item-icon">${esc(item.icon || 'ðŸŽ')}</div>
        <div class="item-name">${esc(item.name)}</div>
        <div class="item-desc">${esc(item.description)}</div>
        ${item.ability ? `<div class="item-ability">âš¡ ${getAbilityName(item.ability)}</div>` : ''}
...
async function renderInventory() {
  ...
  // Plasma consumables â€” shown in same grid with a USE button
  if (hasPlasmaItems) {
    PLASMA_CONSUMABLES.filter(item => (plasmaInv[item.id] || 0) > 0).forEach(item => {
      html += `
        <div class="inv-item-card plasma-inv-card">
          <div class="item-icon">${item.icon}</div>
          <div class="inv-plasma-badge">âš— PLASMA</div>
          <div class="item-name">${item.name} <span style="color:#aa44ff">Ã—${plasmaInv[item.id]}</span></div>
          <div class="item-desc">${item.desc}</div>
          <div class="item-rarity" style="color:${item.rarityColor}">${item.rarity}</div>
          <button class="item-activate-btn" style="background:rgba(102,0,204,.3);border-color:#aa44ff;color:#dd99ff" onclick="usePlasmaItem('${item.id}')">âš— USE</button>
        </div>`;
    });
  }
...
async function buyItem(itemId) {
  ...
  refreshCoins();
  renderItemsShop();
  showToast(`âœ… ${item.name} purchased!`);
}
...
async function activateItem(itemId) {
  ...
  renderInventory();
  showToast(`âœ… ${item ? item.name : 'Item'} activated!`);
}
...
function getAbilityName(ability) {
  const names = {
    bypass_moderation: 'ðŸ”“ Bypass Moderation',
    bypass_reports: 'ðŸ›¡ Bypass Reports',
    coin_boost: 'ðŸ’° +10% Coins',
    double_xp: 'âš¡ 2Ã— XP',
    vip_badge: 'ðŸ‘‘ VIP Badge',
    custom_color: 'ðŸŽ¨ Custom Color',
    infinite_streak: 'ðŸ”¥ Streak Protection'
  };
...
// hasActiveAbility checks this directly â€” no async cache, no race conditions.
    ...
    <div class="dp-item-row">
      <div style="display:flex;align-items:center;gap:10px;flex:1">
        ...
        <div style="flex:1">
          ...
          <div style="font-size:.75rem;color:var(--muted)">${item.price} ðŸ’§ â€¢ ${item.stock > 0 ? (item.stock - (item.purchased || 0)) + ' left' : 'âˆž stock'}</div>
        </div>
      </div>
...
async function sendChat(){
  ...
  if(UC&&UC.exiledUntil>Date.now()){const mins=Math.ceil((UC.exiledUntil-Date.now())/60000);showToast(`ðŸš« You are EXILED! ${mins}m remaining.`);inp.value='';return;}
  // bypass_moderation item overrides mute
  if(UC&&UC.muted&&!hasActiveAbility('bypass_moderation')){showToast('ðŸ”‡ You are muted and cannot chat.');inp.value='';return;}
  inp.value='';
...
function renderChat(){
  ...
  if(!chatCache.length){el.innerHTML='<div class="empty">No messages yet. Say hello! ðŸ‘‹</div>';return;}
  const atBot=el.scrollHeight-el.scrollTop-el.clientHeight<70;
  const me=getU();
  const pinned=chatCache.filter(m=>m.pinned);
  const pinnedBar=pinned.length?`<div class="chat-pinned-bar">ðŸ“Œ <b>${esc(pinned[pinned.length-1].username)}:</b> ${esc(pinned[pinned.length-1].text.slice(0,60))}${pinned[pinned.length-1].text.length>60?'â€¦':''}</div>`:'';
  el.innerHTML=pinnedBar+chatCache.map(m=>{
    ...
    const vtHistory=activeMods.has('ventype')&&_msgHistory[m.id]?_msgHistory[m.id].map(h=>`<div class="vt-history">${h.deleted?'ðŸ—‘ [DELETED]':'âœ '+esc(h.text)} <span style="color:var(--muted);font-size:.65rem">${new Date(h.ts).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</span></div>`).join(''):'';
    ...
    const richIcon=activeMods.has('richpresence')&&m.username===getU()?'<span class="rich-icon">âœ¦</span>':'';
    const pinnedTag=m.pinned?'<span class="pinned-tag">ðŸ“Œ</span>':'';
    const replyHTML=m.replyTo?`<div class="msg-reply-preview" onclick="scrollToMsg('${esca(m.replyTo.id)}')">â†© <b>${esc(m.replyTo.username)}:</b> ${esc((m.replyTo.text||'').slice(0,60))}</div>`:'';
    const actionsOwn=`<button class="mact edit" onclick="chatStartEdit('${esca(m.id)}')">âœ</button><button class="mact del" onclick="chatDelete('${esca(m.id)}')">ðŸ—‘</button>`;
    const actionsAll=`<button class="mact reply" onclick="chatSetReply('${esca(m.id)}','${esca(m.username)}','${esca(m.text.slice(0,60))}')">â†©</button><button class="mact pin" onclick="chatTogglePin('${esca(m.id)}')">${m.pinned?'ðŸ“':'ðŸ“Œ'}</button>`;
...
function chatSetReply(id,username,text){
  ...
  if(bar){bar.style.display='flex';document.getElementById('chat-reply-text').textContent=`â†© ${username}: ${text.slice(0,60)}`;}
...
async function chatTogglePin(id){
  ...
  showToast(m.pinned?'Message unpinned':'ðŸ“Œ Message pinned');
}
...
async function renderLB(){
  ...
  tbody.innerHTML='<tr><td colspan="5" style="text-align:center;padding:18px;color:var(--muted)">Loadingâ€¦</td></tr>';
  ...
  tbody.innerHTML=accs.map((a,i)=>{const bd=a.equippedBadge?ALL_BADGES.find(x=>x.id===a.equippedBadge):null;const bdHTML=bd?`<span title="${esc(bd.name)}" style="margin-left:5px;font-size:.85rem">${bd.icon}</span>`:'';const teamTag=a.teamTag?`<span class="team-tag">[${esc(a.teamTag)}]</span>`:'';const plasmaCell=a.plasma>0?`<td style="color:#cc88ff;font-weight:700;font-size:.88rem">âš— ${a.plasma}</td>`:`<td style="color:var(--muted);font-size:.82rem">â€”</td>`;return `<tr><td><span class="lbrank ${['r1','r2','r3',''][Math.min(i,3)]}">${['ðŸ¥‡','ðŸ¥ˆ','ðŸ¥‰','#'+(i+1)][Math.min(i,3)]}</span></td><td class="lbname" style="cursor:pointer" onclick="openProfile('${esca(a.username)}')">${esc(a.username)}${teamTag}${activeMods&&activeMods.has('richpresence')&&a.username===getU()?'<span class="rich-icon">âœ¦</span>':''}${bdHTML}</td><td class="lbcoins">ðŸ§¢ ${a.coins||0}</td><td style="color:var(--muted);font-size:.82rem">${(a.themes||[]).length} theme${(a.themes||[]).length!==1?'s':''}</td>${plasmaCell}</tr>`;}).join('');
}
...
async function renderAdmAccounts(){
  ...
  tbody.innerHTML='<tr><td colspan="4" style="text-align:center;padding:10px;color:var(--muted)">Loadingâ€¦</td></tr>';
  const accs=await dbAllUsers();
  if(!accs.length){tbody.innerHTML='<tr><td colspan="4" class="empty">No accounts.</td></tr>';return;}
  tbody.innerHTML=accs.map(a=>`<tr><td style="font-weight:700">${esc(a.username)}</td><td class="tdpass" style="filter:blur(5px);user-select:none" title="Hidden for security">â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢</td><td class="tdcoins">ðŸ§¢ ${a.coins||0}</td><td class="tdact"><input class="coinamt" id="ca-${esca(a.username)}" type="number" value="50" min="1" max="99999"><button class="bsm give" onclick="admGive('${esca(a.username)}')">+Give</button><button class="bsm take" onclick="admTake('${esca(a.username)}')">-Take</button><button class="bsm ${a.muted?'unmute':'mute'}" onclick="admToggleMute('${esca(a.username)}')">${a.muted?'ðŸ”ˆ Unmute':'ðŸ”‡ Mute'}</button><button class="bsm del" onclick="admDel('${esca(a.username)}')">ðŸ—‘ Del</button></td></tr>`).join('');
}
async function admGive(u){const amt=parseInt(document.getElementById('ca-'+u).value)||0;if(amt<=0)return;const acc=await dbGetUser(u);if(!acc)return;await dbUpdateUser(u,{coins:(acc.coins||0)+amt});if(u===getU())refreshCoins();showToast(`+${amt} bottlecaps â†’ ${u}`);renderAdmAccounts();}
async function admTake(u){const amt=parseInt(document.getElementById('ca-'+u).value)||0;if(amt<=0)return;const acc=await dbGetUser(u);if(!acc)return;await dbUpdateUser(u,{coins:Math.max(0,(acc.coins||0)-amt)});if(u===getU())refreshCoins();showToast(`-${amt} bottlecaps â† ${u}`);renderAdmAccounts();}
...
async function admToggleMute(u){const acc=await dbGetUser(u);if(!acc)return;const nowMuted=!acc.muted;await dbUpdateUser(u,{muted:nowMuted});showToast(nowMuted?`ðŸ”‡ ${u} muted`:`ðŸ”ˆ ${u} unmuted`);renderAdmAccounts();}
  ...
  el.innerHTML=chatCache.map(m=>{
    ...
    return `<div class="mcmsg" id="adm-msg-${m.id}">
      ...
      <div class="mcmsg-actions">
        <button class="bsm edit" onclick="admStartEdit('${esca(m.id)}')">âœ Edit</button>
        <button class="bsm rm" onclick="modDel('${esca(m.id)}','adm')">ðŸ—‘ Del</button>
      </div>
    </div>`;
  }).join('');
}
...
async function admSaveEdit(id){const v=document.getElementById('adm-einp-'+id).value.trim();if(!v){showToast('Cannot be empty.');return;}await dbEditMsg(id,v);showToast('Message edited âœ“');}
...
// â”€â”€ DEPOULE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    el.innerHTML=snap.docs.map(d=>{
      ...
      return `<div class="report-item" id="rpt-${d.id}">
        <div class="report-header">
          <div class="report-accused">ðŸš© <span style="font-weight:700;color:var(--accent2)">${esc(r.accused)}</span></div>
        ...
        ${r.status==='pending'?`<div class="report-actions">
          <button class="bsm punish" onclick="reportPunish('${d.id}','${esca(r.accused)}')">âš¡ Punish</button>
          <button class="bsm forgive" onclick="reportForgive('${d.id}')">âœ… Forgive</button>
          <button class="bsm del" onclick="reportDismiss('${d.id}')">ðŸ—‘ Dismiss</button>
        </div>`:''}
      </div>`;
    }).join('');
...
function renderDPChat(){
  ...
  el.innerHTML=chatCache.map(m=>{
    ...
    return `<div class="mcmsg" id="dp-msg-${m.id}">
      ...
      <div class="mcmsg-actions">
        <button class="bsm edit" onclick="dpStartEdit('${esca(m.id)}')">âœ Edit</button>
        <button class="bsm rm" onclick="modDel('${esca(m.id)}','dp')">ðŸ—‘ Del</button>
      </div>
    </div>`;
  }).join('');
}
...
async function dpSaveEdit(id){const v=document.getElementById('dp-einp-'+id).value.trim();if(!v){showToast('Cannot be empty.');return;}await dbEditMsg(id,v);showToast('Message edited âœ“');}

// â”€â”€ PROFILE MODAL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const THEME_COLORS={
...
async function openProfile(username){
  ...
  document.getElementById('prof-name').textContent='Loadingâ€¦';
  document.getElementById('prof-coins').textContent='â€¦';
  document.getElementById('prof-streak').textContent='â€¦';
  document.getElementById('prof-themes').textContent='â€¦';
  document.getElementById('prof-theme-row').innerHTML='';
  document.getElementById('prof-actions').innerHTML='<div style="color:var(--muted);text-align:center;padding:8px;font-size:.88rem">Loadingâ€¦</div>';
  ...
  badgeEl.textContent = `${badge.label} â€¢ ${feudalRank}`;
  ...
  if(xrayEl){
    if(activeMods&&activeMods.has('xray')){
      ...
      xrayEl.innerHTML=`<div style="margin-top:12px;padding:10px 14px;background:rgba(255,136,0,.07);border:1px solid rgba(255,136,0,.2);border-radius:8px;font-size:.8rem">
        <div style="color:#ff8800;font-family:'Bebas Neue',cursive;letter-spacing:2px;margin-bottom:6px">ðŸ” XRAY DATA</div>
  ...
  if(isSelf){
    actEl.innerHTML=`<div class="prof-self-note">This is your profile! Earn coins by racing ðŸ</div>`;
  } else {
    actEl.innerHTML=`
      ...
      <div class="gift-row">
        <input class="gift-input" id="gift-amt" type="number" min="1" placeholder="Amountâ€¦" value="10">
        <button class="gift-btn" id="gift-btn" onclick="giftCoins()">ðŸŽ Gift</button>
      </div>
      <div style="font-size:.75rem;color:var(--muted);margin-top:5px">Your balance: ðŸ§¢ ${UC?UC.coins:0} bottlecaps</div>
      <button onclick="openDMWith('${esca(acc.username)}')" style="width:100%;margin-top:10px;padding:10px;border:none;border-radius:8px;background:linear-gradient(135deg,#003366,#0055aa);color:#fff;font-family:'Rajdhani',sans-serif;font-size:.95rem;font-weight:700;letter-spacing:1px;cursor:pointer;">âœ‰ Message</button>
      <button onclick="openReportModal('${esca(acc.username)}')" style="width:100%;margin-top:8px;padding:10px;border:none;border-radius:8px;background:rgba(200,0,0,.15);border:1px solid rgba(200,0,0,.3);color:#ff6666;font-family:'Rajdhani',sans-serif;font-size:.95rem;font-weight:700;letter-spacing:1px;cursor:pointer;">ðŸš© Report</button>
      <button onclick="openTrollModal('${esca(acc.username)}')" style="width:100%;margin-top:8px;padding:10px;border:none;border-radius:8px;background:rgba(255,140,0,.1);border:1px solid rgba(255,140,0,.3);color:#ffaa44;font-family:'Rajdhani',sans-serif;font-size:.95rem;font-weight:700;letter-spacing:1px;cursor:pointer;">ðŸŽ­ Troll</button>
      ${(getU() === FS.king && !isSelf) ? `
        <div style="border:1px solid #00aaff;padding:12px;border-radius:10px;margin-top:10px;background:rgba(0,170,255,.05)">
          <div style="font-family:'Bebas Neue',cursive;font-size:1rem;color:#00aaff;margin-bottom:8px;letter-spacing:1px">ðŸ‘‘ Royal Appointment</div>
      ...
      ${(feudalRank === 'Serf' && !acc.master && getU() !== FS.king) ? `<button onclick="sendSlaveryRequest()" style="width:100%;margin-top:8px;padding:10px;border:none;border-radius:8px;background:rgba(139,69,19,.2);border:1px solid #8b4513;color:#d2b48c;font-family:'Rajdhani',sans-serif;font-size:.95rem;font-weight:700;letter-spacing:1px;cursor:pointer;">ðŸ“œ Request Serfdom</button>` : ''}
      ${(feudalRank === 'King' && getU() !== FS.king) ? 
        `<button onclick="castRevoltVote()" style="width:100%;margin-top:8px;padding:10px;border:none;border-radius:8px;background:#300;border:1px solid #f00;color:#f44;font-weight:700;cursor:pointer;">âš” REVOLT</button>` 
        : ''}
    `;
  }
}
...
async function giftCoins(){
  ...
  if(!target){showToast('User not found.');btn.disabled=false;btn.textContent='ðŸŽ Gift';return;}
  // Deduct from self
  UC.coins=(UC.coins||0)-amt;
  await dbUpdateUser(getU(),{coins:UC.coins});
  // Add to target
  ...
  showToast(`Gifted ðŸ§¢ ${amt} bottlecaps to ${profileTarget}!`);
  closeProfile();
}

// â”€â”€ DEPOULE PET BUTTON â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
let petState={color:'green',timer:null,wins:0,losses:0,pets:0,net:0,combo:0,goodPetStreak:0,rageMode:false,cooldown:false};
const RAGE_MESSAGES=['DePoule is FURIOUS ðŸ˜¡','IT BURNS ðŸ”¥','RUN. ðŸ’€','THE ENTITY RAGES','PAIN IS COMING'];
const WIN_MESSAGES=['Nice pet ðŸ˜Œ','It approvesâ€¦','Luckyâ€¦','It liked that','Blessed ðŸ€','DePoule purrsâ€¦','Combo! âš¡'];
const LOSE_MESSAGES=['It bit you ðŸ˜¡','OUCH ðŸ’€','DePoule attacks!','Bad timing!','It feeds on you','PUNISHED','You fool ðŸ’€'];
...
function schedulePetFlip(){
  ...
  petState.timer=setTimeout(()=>{
    ...
    btn.textContent=petState.color==='green'?'ðŸ¾ PET':'âš  PET';
    const hint=document.getElementById('pet-hint');
    if(hint){
      if(petState.rageMode){hint.className='pet-hint bad';hint.textContent='RAGE MODE â€” 75% red!';}
      else{hint.className='pet-hint '+(petState.color==='green'?'good':'bad');hint.textContent=petState.color==='green'?'ðŸŸ¢ Green â€” pet now!':'ðŸ”´ Red â€” danger!';}
    }
    schedulePetFlip();
  },getFlipDelay());
}
async function petDePoule(){
  ...
  if(won){
    ...
    if(isJackpot){res.textContent='JACKPOT +'+finalCoinGain+'ðŸª™';res.className='pet-result jackpot';showToast('JACKPOT!! +'+finalCoinGain+' coins! ðŸŽ°');}
    else{res.textContent='+'+(finalCoinGain>1?finalCoinGain+' ðŸ§¢':'1 ðŸª™');res.className='pet-result win';}
    const hint=document.getElementById('pet-hint');
    if(hint){hint.className='pet-hint '+(isJackpot?'jackpot-hint':'good');hint.textContent=isJackpot?'JACKPOT!! ðŸŽ°':WIN_MESSAGES[Math.floor(Math.random()*WIN_MESSAGES.length)]+(petState.combo>1?' (x'+petState.combo+'!)':'');}
    ...
    if(petState.goodPetStreak>0&&petState.goodPetStreak%50===0){
      ...
      showToast(`ðŸ¦† ${petState.goodPetStreak} good pets! ${disc}% shop discount active!`);
    }
    updateStreakBar();
  } else {
    ...
    await dbUpdateUser(getU(),{coins:UC.coins});refreshCoins();
    const res=document.getElementById('pet-result');
    res.textContent=bigLoss?'PUNISHED âˆ’'+coinLoss+'ðŸª™':'âˆ’1 ðŸª™';res.className='pet-result lose';
    const hint=document.getElementById('pet-hint');
    ...
    if(bigLoss){
      ...
      showToast('DePoule ENTERS RAGE MODE! ðŸ˜¡ðŸ”¥');
  ...
  updatePetUI();
  setTimeout(()=>{const res=document.getElementById('pet-result');if(res)res.textContent='';if(!petState.rageMode){const hint=document.getElementById('pet-hint');if(hint){hint.className='pet-hint neutral';hint.textContent='Pet DePouleâ€¦ if you dare';}}},1400);
}
...
function getMoodText(){if(petState.pets===0)return 'Dormantâ€¦';const r=petState.wins/petState.pets;if(r>0.7)return 'Content ðŸ˜Œ';if(r>0.5)return 'Neutralâ€¦';if(r>0.3)return 'Irritated ðŸ˜¤';return 'Hostile ðŸ˜¡';}
function updatePetUI(){
  ...
  if(cl)cl.textContent=petState.combo>0?'Combo: '+petState.combo+'x â€” '+(petState.combo>=10?'JACKPOT READY ðŸŽ°':petState.combo>=5?'+3 per pet':petState.combo>=3?'+2 per pet':'+1 per pet'):'Combo: 0x';
...
function playDMSound() {
  try {
    ...
    // Two-tone chime â€” friendly notification sound
...
function renderDMList(){
  ...
  if(!convos.length){
    el.innerHTML='<div class="empty" style="padding:24px;text-align:center;font-size:.88rem">No conversations yet.<br><span style="color:var(--muted);font-size:.8rem">Open a profile and click âœ‰ Message</span></div>';
    return;
  }
...
function dmSetReply(msgId,from,text){
  ...
  if(bar){bar.style.display='flex';document.getElementById('dm-reply-text').textContent=`â†© ${from}: ${text.slice(0,60)}`;}
...
// â”€â”€ HUB â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function renderBank() {
  ...
  if (loan) {
    ...
    document.getElementById('bl-original').textContent       = `ðŸ§¢ ${loan.principal.toLocaleString()}`;
    ...
    document.getElementById('bl-total').textContent          = `ðŸ§¢ ${total.toLocaleString()}`;
...
function updateBankPreview() {
  ...
  set('bp-receive', amt > 0 ? `${amt.toLocaleString()} ðŸ§¢` : 'â€”');
  set('bp-day1', amt > 0 ? `${owed(0).toLocaleString()} ðŸ§¢` : 'â€”');
  set('bp-day2', amt > 0 ? `${owed(1).toLocaleString()} ðŸ§¢` : 'â€”');
  set('bp-day3', amt > 0 ? `${owed(2).toLocaleString()} ðŸ§¢` : 'â€”');
  set('bp-day4', amt > 0 ? `${owed(3).toLocaleString()} ðŸ§¢` : 'â€”');
}

async function bankTakeLoan() {
  if (!UC) return;
  if (getBankLoan()) { showToast('âŒ Repay your current loan first!'); return; }

  const inp = document.getElementById('bank-loan-input');
  const amt = parseInt(inp?.value) || 0;

  if (amt < BANK_MIN_LOAN) { showToast(`âŒ Minimum loan is ${BANK_MIN_LOAN} ðŸ§¢`); return; }
  if (amt > BANK_MAX_LOAN) { showToast(`âŒ Maximum loan is ${BANK_MAX_LOAN.toLocaleString()} ðŸ§¢`); return; }
  ...
  refreshCoins();
  renderBank();
  showToast(`ðŸ¦ Loaned ${amt.toLocaleString()} ðŸ§¢! Pay it back before interest doubles!`);
}
...
async function bankRepayFull() {
  ...
  if (bal < total) {
    showToast(`âŒ Not enough caps! You need ${total.toLocaleString()} ðŸ§¢ but have ${bal.toLocaleString()} ðŸ§¢.`);
    return;
  }

  const confirmed = confirm(`Repay full loan of ðŸ§¢ ${total.toLocaleString()} (principal ðŸ§¢ ${loan.principal.toLocaleString()} + interest ðŸ§¢ ${(total - loan.principal).toLocaleString()})?`);
  if (!confirmed) return;
  ...
  refreshCoins();
  renderBank();
  showToast(`âœ… Loan repaid! ðŸ¦ You're debt-free.`);
}
...
async function bankRepayPartial() {
  ...
  const input = prompt(`How many caps to repay? (You owe ðŸ§¢ ${total.toLocaleString()}, you have ðŸ§¢ ${bal.toLocaleString()})\nMax you can pay: ðŸ§¢ ${maxPay.toLocaleString()}`, maxPay);
  if (!input) return;

  const paying = Math.floor(parseInt(input) || 0);
  if (paying <= 0) { showToast('âŒ Enter a valid amount.'); return; }
  if (paying > bal) { showToast(`âŒ You only have ðŸ§¢ ${bal.toLocaleString()}.`); return; }
  if (paying > total) { showToast(`âŒ You only owe ðŸ§¢ ${total.toLocaleString()}.`); return; }
  ...
  if (newPrincipal <= 0) {
    ...
    UC.bankLoan = null;
    await dbUpdateUser(getU(), { bankLoan: null, coins: UC.coins });
    showToast('âœ… Loan fully repaid!');
  } else {
    // Partially paid â€” update principal & reset takenAt so interest resets from now
    const newLoan = { principal: newPrincipal, takenAt: Date.now() };
    UC.bankLoan = newLoan;
    await dbUpdateUser(getU(), { bankLoan: newLoan, coins: UC.coins });
    showToast(`ðŸ’¸ Paid ðŸ§¢ ${paying.toLocaleString()}. Remaining loan: ðŸ§¢ ${newPrincipal.toLocaleString()}`);
  }
...
function checkBankLoanOnLogin() {
  ...
  if (days >= 1) {
    ...
    showToast(`âš ï¸ Bank: Your loan interest is now ${rate}%! You owe ðŸ§¢ ${total.toLocaleString()}. Pay now!`);
  }
}
...
function renderDMConvo(id){
  ...
  const pinnedBar=pinnedMsgs.length?`<div class="dm-pinned-bar">ðŸ“Œ <b>${esc(pinnedMsgs[pinnedMsgs.length-1].from)}:</b> ${esc(pinnedMsgs[pinnedMsgs.length-1].text.slice(0,60))}${pinnedMsgs[pinnedMsgs.length-1].text.length>60?'â€¦':''}</div>`:'';
  el.innerHTML=pinnedBar+convo.messages.slice(-150).map(m=>{
    ...
    const replyHTML=m.replyTo?`<div class="dm-reply-preview">â†© <b>${esc(m.replyTo.from)}:</b> ${esc((m.replyTo.text||'').slice(0,50))}</div>`:'';
    const ownBtns=mine?`<button class="dm-act-btn" onclick="dmStartEdit('${esca(m.id)}')">âœ</button><button class="dm-act-btn dm-act-del" onclick="dmDeleteMsg('${esca(m.id)}')">ðŸ—‘</button>`:'';
    const sharedBtns=`<button class="dm-act-btn" onclick="dmSetReply('${esca(m.id)}','${esca(m.from)}','${esca(m.text.slice(0,60))}')">â†©</button><button class="dm-act-btn" onclick="dmTogglePin('${esca(id)}','${esca(m.id)}')" title="${isPinned?'Unpin':'Pin'}">${isPinned?'ðŸ“':'ðŸ“Œ'}</button>`;
...
async function sendDM(){
  ...
  if(!text)return;
  if(UC&&UC.muted){showToast('ðŸ”‡ You are muted and cannot send DMs.');inp.value='';return;}
  if(!FB_READY){showToast('DMs require Firebase.');return;}
  inp.value='';
  ...
  el.innerHTML=updateLogCache.map(u=>`
    <div class="mgr-entry">
      ...
      <div class="mgr-entry-actions">
        <button class="bsm edit" onclick="mgrEdit('${esca(u.id)}')">âœ Edit</button>
        <button class="bsm del" onclick="mgrDelete('${esca(u.id)}')">ðŸ—‘ Del</button>
      </div>
    </div>`).join('');
}
...
// â”€â”€ QUEST & SECRET THEME SYSTEM â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const SECRET_QUEST_DATA = {
  ...
  voidwalker: {
    name:'Void Walker',
    how:'Log in 7 days in a row (streak â‰¥ 7)',
...
async function checkAndGrantSecretThemes(wpm){
  ...
  // Void Walker: 7-day streak â€” only check after login, not on wpm=0 startup calls
...
// â”€â”€ EASTER EGG SYSTEM â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const EGG_CODES = {
  ...
  'liquidtype': ()=>{
    showToast('ðŸ You found the hidden cheer!');
    confettiBlast('#cc0000');
    for(let i=0;i<5;i++)setTimeout(()=>showToast('ðŸ'),i*400);
  },
  'konami': ()=>{
    if(UC){UC.coins=(UC.coins||0)+50;dbUpdateUser(getU(),{coins:UC.coins});refreshCoins();}
    showToast('ðŸŽ® Konami Code: +50 bottlecaps!');
  },
  'depouleisreal': ()=>{
    showToast('ðŸ‘ï¸ It sees you.');
  },
  'ggobsiscool': ()=>{
    if(UC){UC.coins=(UC.coins||0)+100;dbUpdateUser(getU(),{coins:UC.coins});refreshCoins();}
    showToast('ðŸ† Ggobs blesses you: +100 coins!');
  },
  'holographic': ()=>{
    showToast('âœ¨ You see through the veil.');
    document.body.style.animation='holoShift 1s infinite';
    setTimeout(()=>document.body.style.animation='',5000);
  },
  'zerozero': ()=>{
    showToast("ðŸ”¢ The void between numbers.");
...
function renderSettings(){
  ...
  if(musicBtn){musicBtn.textContent=s.music?'ðŸ”Š On':'ðŸ”‡ Off';musicBtn.className='st-toggle'+(s.music?' on':'');}
  if(effectsBtn){effectsBtn.textContent=s.effects?'âœ… On':'âŒ Off';effectsBtn.className='st-toggle'+(s.effects?' on':'');}
  if(epilepsyBtn){epilepsyBtn.textContent=s.epilepsy?'âš¡ Enabled':'ðŸ’¤ Disabled';epilepsyBtn.className='st-toggle'+(s.epilepsy?' warn':'');}
...
// â”€â”€ REDEEM CODES â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function redeemCode(){
  ...
  try{
    ...
    if(!doc.exists){showToast('âŒ Invalid code.');btn.disabled=false;btn.textContent='Redeem';return;}
    const data=doc.data();
    ...
    showToast('âœ… Code redeemed! '+msg);
  } catch(e){
    console.error(e);
...
// â”€â”€ DEPOULE CODE CREATOR â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function dpCreateCode(){
  ...
  showToast('âœ… Code created: '+codeVal);
...
async function renderDPCodes(){
  ...
  el.innerHTML=snap.docs.map(d=>{
    ...
    const max=c.maxUses?`/${c.maxUses}`:'âˆž';
    const reward=c.type==='coins'?`ðŸ§¢ ${c.amount} bottlecaps`:c.type==='theme'?`ðŸŽ¨ ${c.theme}`:c.type==='badge'?`ðŸ… badge:${c.badgeId}`:`ðŸŽ’ items`;
    return `<div class="dp-code-row">
      ...
      <button class="bsm del" onclick="dpDeleteCode('${esca(d.id)}')">ðŸ—‘</button>
    </div>`;
  }).join('');
}
...
const ALL_BADGES = [
  {id:'first_race',   icon:'ðŸ', name:'First Lap',      desc:'Complete your first race.',         secret:false},
  {id:'win_race',     icon:'ðŸ¥‡', name:'Winner',         desc:'Finish 1st place in a race.',       secret:false},
  {id:'streak3',      icon:'ðŸ”¥', name:'On Fire',        desc:'Log in 3 days in a row.',           secret:false},
  {id:'streak7',      icon:'ðŸ”¥ðŸ”¥', name:'Inferno',      desc:'Log in 7 days in a row.',           secret:false},
  {id:'caps100',      icon:'ðŸ§¢', name:'Pocket Change',  desc:'Earn 100 bottlecaps total.',        secret:false},
  {id:'caps1000',     icon:'ðŸ’°', name:'Bottlecap Baron',desc:'Earn 1000 bottlecaps total.',       secret:false},
  {id:'caps5000',     icon:'ðŸ‘‘', name:'Cap King',       desc:'Earn 5000 bottlecaps total.',       secret:false},
  {id:'themes5',      icon:'ðŸŽ¨', name:'Collector',      desc:'Own 5 themes.',                     secret:false},
  {id:'themes15',     icon:'ðŸ–¼', name:'Connoisseur',    desc:'Own 15 themes.',                    secret:false},
  {id:'wpm80',        icon:'âš¡', name:'Speed Typist',   desc:'Type at 80+ WPM.',                  secret:false},
  {id:'wpm100',       icon:'ðŸš€', name:'Ludicrous Speed',desc:'Type at 100+ WPM.',                 secret:false},
  {id:'live_win',     icon:'ðŸŒ', name:'Net Champion',   desc:'Win a live race.',                  secret:false},
  {id:'gifter',       icon:'ðŸŽ', name:'Generous',       desc:'Gift bottlecaps to another player.',secret:false},
  {id:'reporter',     icon:'ðŸš©', name:'Watchdog',       desc:'Submit a report.',                  secret:false},
  {id:'depoule_pet',  icon:'ðŸ¾', name:'Tamed',          desc:'Pet DePoule 50 times.',             secret:false},
  {id:'jackpot',      icon:'ðŸŽ°', name:'Jackpot!',       desc:'Hit a DePoule jackpot.',            secret:false},
  // SECRET BADGES â€” hidden until unlocked
  {id:'alucard',      icon:'ðŸ§›', name:'ALUCARD',        desc:'???',                               secret:true},
  {id:'void_walker',  icon:'ðŸŒ‘', name:'Void Walker',    desc:'???',                               secret:true},
  {id:'depoule_chosen',icon:'ðŸ¦†',name:'Chosen by DePoule',desc:'???',                            secret:true},
];
...
async function grantBadge(id){
  ...
  if(b){
    ...
    n.innerHTML='<div style="font-size:.68rem;letter-spacing:2px;text-transform:uppercase;color:#ffd700;margin-bottom:4px">ðŸ… Badge Unlocked</div><div style="font-size:1rem;font-weight:700">'+(b.icon)+' '+esc(b.name)+'</div><div style="font-size:.75rem;color:rgba(255,255,255,.5);margin-top:2px">'+esc(b.desc)+'</div>';
...
function renderBadges(){
  ...
  el.innerHTML=visible.map(b=>{
    ...
    return `<div class="badge-card${isEquipped?' equipped':''}">
      ...
      ${isEquipped
        ? `<button class="badge-equip-btn unequip" onclick="unequipBadge()">âœ— Unequip</button>`
...
// â”€â”€ WORD FILTER â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function dpSaveBannedWords(){
  ...
  showToast(`âœ… Word filter saved (${words.length} word${words.length!==1?'s':''})`);
}
...
// â”€â”€ TROLL SYSTEM â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const TROLL_ACTIONS = [
  {id:'orbital',  icon:'ðŸ›°ï¸', label:'Orbital Strike Cannon',  cost:200, desc:'Fires an orbital strike â€” their screen goes completely insane for 20 seconds.'},
  {id:'theme',    icon:'ðŸŽ¨', label:'Force ugly theme',       cost:75,  desc:'Forces their theme to "Ash" (grey) for 5 minutes.'},
  {id:'slowmode', icon:'ðŸ¢', label:'TROLL VICTIM label',     cost:50,  desc:'Stamps (TROLL VICTIM) on their display name for 15 mins.'},
  {id:'confetti', icon:'ðŸŽ‰', label:'Spam confetti at them',  cost:30,  desc:'Sends them a surprise confetti popup notification.'},
  {id:'rename',   icon:'ðŸ“›', label:'Give them a nickname',   cost:150, desc:'Adds a custom ALL-CAPS prefix to their name for 10 mins.'},
  {id:'forcemsg', icon:'ðŸ’¬', label:'Make them say something',cost:25,  desc:'Posts a message as them â€” shows (trolled) tag so everyone knows.'},
  {id:'flip',     icon:'ðŸ™ƒ', label:'Flip their screen',      cost:60,  desc:'Turns their whole page upside-down for 20 seconds.'},
  {id:'shake',    icon:'ðŸ’¥', label:'Shake their screen',     cost:40,  desc:'Makes their screen violently shake for 15 seconds.'},
  {id:'jumpscare',icon:'ðŸ‘»', label:'Jumpscare',              cost:80,  desc:'Sends them a scary popup notification they have to dismiss.'},
  {id:'darkmode', icon:'ðŸŒ‘', label:'Force Void theme',       cost:70,  desc:'Forces their theme to Void (almost pitch black) for 5 mins.'},
];
...
function renderTrollActions(){
  ...
  el.innerHTML=TROLL_ACTIONS.map(a=>{
    ...
    return `<div class="troll-action${canAfford?'':' cant-afford'}">
      ...
      <button class="troll-buy-btn" onclick="executeTroll('${a.id}')" ${canAfford?'':'disabled'}>ðŸ§¢ ${a.cost}</button>
    </div>`;
  }).join('');
}
...
async function executeTroll(actionId){
  ...
  if(actionId==='orbital'){
    await dbUpdateUser(trollTarget,{trollOrbital:true,trollOrbitalUntil:now+20000,trollNotif:{by:trolledBy,action:'fired the Orbital Strike Cannon at you ðŸ›°ï¸',ts:now}});
    ...
    toastMsg=`ðŸ›°ï¸ ORBITAL STRIKE fired at ${trollTarget}!`;
  }
    ...
    toastMsg=`ðŸŽ¨ Forced ${trollTarget}'s theme to Ash!`;
  }
    ...
    toastMsg=`ðŸ¢ ${trollTarget} is now a TROLL VICTIM!`;
  }
  else if(actionId==='confetti'){
    await dbUpdateUser(trollTarget,{trollNotif:{by:trolledBy,action:'blasted confetti in your face ðŸŽ‰',ts:now}});
    toastMsg=`ðŸŽ‰ Confetti sent to ${trollTarget}!`;
  }
    ...
    toastMsg=`ðŸ’¬ ${trollTarget} now says "${forcedMsg.slice(0,30)}"!`;
  }
    ...
    toastMsg=`ðŸ™ƒ ${trollTarget}'s screen flipped!`;
  }
    ...
    toastMsg=`ðŸ’¥ ${trollTarget}'s screen is shaking!`;
  }
    ...
    toastMsg=`ðŸ‘» Jumpscare sent to ${trollTarget}!`;
  }
    ...
    toastMsg=`ðŸŒ‘ ${trollTarget}'s screen went dark!`;
  }
    ...
    toastMsg=`ðŸ“› ${trollTarget} is now "${safeNick}"!`;
  }
...
function showTrollNotif(notif){
  ...
  if(notif.jumpscare){
    ...
    s.innerHTML=`<div style="font-size:8rem;animation:jumpscareGrow .4s ease">ðŸ‘»</div><div style="font-family:'Bebas Neue',cursive;font-size:3rem;letter-spacing:6px;color:#ff0000;text-shadow:0 0 30px #f00;animation:jumpscareGrow .3s ease">BOO!</div><div style="color:rgba(255,255,255,.6);margin-top:12px;font-size:1rem">${esc(notif.by)} jumpscared you</div><button onclick="this.parentElement.remove();dbUpdateUser(getU(),{trollNotif:null})" style="margin-top:20px;padding:10px 30px;border:none;border-radius:8px;background:#cc0000;color:#fff;font-family:'Rajdhani',sans-serif;font-size:1rem;font-weight:700;cursor:pointer">ðŸ˜¤ OK</button>`;
...
// â”€â”€ MODS SYSTEM â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
let MOD_PW='';
...
const ALL_MODS=[
  ...
  },
  {
    ...
    icon:'ðŸ’Ž',
    name:'RPC',
    desc:'Shows a âœ¦ diamond icon next to your name in chat and leaderboard while active.',
    color:'#ffd700',
  },
  {
    id:'nightowl',
  ...
  {id:'compact',icon:'ðŸ“',name:'CompactMode',desc:'Reduces padding and font sizes throughout the UI for a denser, more info-dense view.',color:'#aaaaaa'},
  {id:'timestamps',icon:'ðŸ•',name:'FullTimestamps',desc:'Chat shows full date+time (Jan 15, 2:34 PM) instead of just the time.',color:'#88aaff'},
  {id:'chatbubbles',icon:'ðŸ’¬',name:'BubbleChat',desc:'Chat messages appear as rounded bubbles instead of flat rows. Your messages appear on the right.',color:'#ff88aa'},
  {id:'largetype',icon:'ðŸ” ',name:'LargeType',desc:'Increases all chat text to 1.15Ã— size. Great for readability.',color:'#ffcc44'},
  {id:'smoothscroll',icon:'ðŸŒŠ',name:'SmoothScroll',desc:'Chat auto-scrolls smoothly to new messages with an animation instead of snapping.',color:'#44ddff'},
  {id:'mutedsounds',icon:'ðŸ””',name:'PingSound',desc:'Plays a soft ping sound when a new chat message arrives.',color:'#88ff88'},
  {id:'rainbowname',icon:'ðŸŒˆ',name:'RainbowName',desc:'Your username in chat cycles through rainbow colors.',color:'#ff44ff'},
  {id:'hidejoins',icon:'ðŸ‘¤',name:'HideOtherUsers',desc:'Only shows messages from yourself and one specific user. All others are hidden.',color:'#cc6600'},
  {id:'wordcount',icon:'ðŸ“Š',name:'WordCounter',desc:'Shows a live word count and estimated reading time on every chat message.',color:'#44ffaa'},
  {id:'fontmono',icon:'âŒ¨',name:'MonoFont',desc:'Forces JetBrains Mono monospace font on all chat text.',color:'#cccccc'},
  {id:'invert',icon:'â¬›',name:'InvertColors',desc:'Inverts the entire page color scheme. Great for accessibility.',color:'#ffffff'},
  {id:'blur_bg',icon:'ðŸŒ«',name:'BlurBG',desc:'Adds a frosted glass blur effect to all cards and panels.',color:'#aaccff'},
  {id:'zoom',icon:'ðŸ”',name:'UIZoom',desc:'Zooms the entire UI to 110% for easier reading on small screens.',color:'#ffaa88'},
  {id:'streakflame',icon:'ðŸ”¥',name:'StreakFlame',desc:'Adds an animated fire emoji next to your streak count on the leaderboard.',color:'#ff6600'},
  {id:'hidechat',icon:'ðŸ™ˆ',name:'FocusMode',desc:'Hides the chat tab entirely so you can focus on racing without distractions.',color:'#888888'},
  {id:'pingmention',icon:'ðŸ“£',name:'Mentions',desc:'Highlights any chat message that contains your username in bright gold.',color:'#ffd700'},
  {id:'autorefresh',icon:'ðŸ”„',name:'AutoRefreshLB',desc:'Leaderboard automatically refreshes every 30 seconds.',color:'#44ffdd'},
  {id:'confettiwin',icon:'ðŸŽŠ',name:'WinConfetti',desc:'Triggers a confetti burst on your screen every time you finish 1st in a race.',color:'#ff88ff'},
  {id:'hidead',icon:'ðŸš«',name:'CleanView',desc:'Removes visual noise: hides the MOTD bar, Discord button, and other nav clutter.',color:'#ff4444'},
  {id:'bigavatar',icon:'ðŸ…°',name:'BigAvatars',desc:'Makes chat avatars 48px instead of 32px for a more visual chat experience.',color:'#ffaa00'},
];
...
function renderModsList(){
  ...
  el.innerHTML=ALL_MODS.map(m=>{
    ...
    return `<div class="mod-card${on?' mod-on':''}">
      ...
      <button class="mod-toggle-btn ${on?'on':'off'}" onclick="toggleMod('${m.id}')">${on?'âœ… ON':'â¬œ OFF'}</button>
    </div>`;
  }).join('');
  applyAllMods();
}
...
// â”€â”€ DP CUSTOM THEME PUBLISHER â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function dpPublishTheme() {
  ...
  showToast('âœ… Theme "' + name + '" published!');
  dpPreviewTheme();
...
async function renderDPPublishedThemes() {
  ...
  el.innerHTML = dpPublishedThemesCache.map(t => `
    <div class="dp-pub-theme" style="border-left:4px solid ${t.acc||'#888'}">
      ...
      <div style="font-size:.72rem;color:var(--muted)">${esc(t.desc)} Â· ðŸ’§${t.price} Â· anim:${t.anim||'none'}</div>
      <button class="bsm del" onclick="dpDeleteTheme('${esca(t.id)}')" style="margin-top:4px">ðŸ—‘ Remove</button>
    </div>`).join('');
}
...
async function loadDPThemesIntoShop() {
  ...
  dpPublishedThemesCache.forEach(t => {
    ...
    else act = `<div class="tprice">ðŸ’§ ${t.price}</div><button class="tbuy" onclick="buyDPTheme('${esca(t.id)}',${t.price})" ${(UC.coins||0)<t.price?'disabled':''}>Buy & Equip</button>`;
...
// â”€â”€ APS PANEL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
let APS_PW = '';
let apsOpen = false;
...
// 
// To set passwords, use the APS Panel â†’ Passwords section
...
async function renderAPSAccounts() {
  ...
  el.innerHTML = accs.map(a => `
    <div class="aps-acc-row">
      <div class="aps-acc-name">${esc(a.username)} <span style="color:var(--muted);font-size:.75rem">${a.muted ? 'ðŸ”‡' : ''}</span></div>
      <div class="aps-acc-coins">ðŸ§¢ ${a.coins || 0}</div>
      <div class="aps-acc-acts">
        ...
        <button class="bsm ${a.muted ? 'unmute' : 'mute'}" onclick="apsToggleMute('${esca(a.username)}')">${a.muted ? 'ðŸ”ˆ Unmute' : 'ðŸ”‡ Mute'}</button>
        <button class="bsm give" onclick="apsResetStreak('${esca(a.username)}')">ðŸ”„ Streak</button>
        <button class="bsm del" onclick="apsDel('${esca(a.username)}')">ðŸ—‘ Del</button>
      </div>
    </div>`).join('');
}

async function apsGive(u) { const amt=parseInt(document.getElementById('aps-ca-'+u).value)||0; if(amt<=0)return; const acc=await dbGetUser(u); if(!acc)return; await dbUpdateUser(u,{coins:(acc.coins||0)+amt}); if(u===getU())refreshCoins(); showToast(`+${amt} ðŸ§¢ â†’ ${u}`); renderAPSAccounts(); }
async function apsTake(u) { const amt=parseInt(document.getElementById('aps-ca-'+u).value)||0; if(amt<=0)return; const acc=await dbGetUser(u); if(!acc)return; await dbUpdateUser(u,{coins:Math.max(0,(acc.coins||0)-amt)}); if(u===getU())refreshCoins(); showToast(`-${amt} ðŸ§¢ â† ${u}`); renderAPSAccounts(); }
...
async function apsToggleMute(u) { const acc=await dbGetUser(u); if(!acc)return; await dbUpdateUser(u,{muted:!acc.muted}); showToast(!acc.muted?`ðŸ”‡ ${u} muted`:`ðŸ”ˆ ${u} unmuted`); renderAPSAccounts(); }
...
function renderAPSChat() {
  ...
  el.innerHTML = chatCache.map(m => {
    ...
    return `<div class="mcmsg">
      ...
      <div class="mcmsg-actions">
        <button class="bsm rm" onclick="apsDelMsg('${esca(m.id)}')">ðŸ—‘ Del</button>
      </div>
    </div>`;
  }).join('');
}
  ...
  db.collection('reports').orderBy('ts','desc').limit(100).get().then(snap => {
    ...
    el.innerHTML = snap.docs.map(d => {
      ...
      return `<div class="report-item">
        <div class="report-header">
          <span style="font-weight:700;color:var(--accent2)">ðŸš© ${esc(r.accused)}</span>
          <span style="color:var(--muted);font-size:.72rem"> â€” by ${esc(r.reporter)}</span>
...
async function renderAPSCodes() {
  ...
  el.innerHTML = snap.docs.map(d => {
    const c = d.data();
    const reward = c.type==='coins'?`ðŸ§¢ ${c.amount}`:c.type==='theme'?`ðŸŽ¨ ${c.theme}`:c.type==='badge'?`ðŸ… ${c.badgeId}`:'ðŸŽ’ items';
    return `<div class="dp-code-row">
      <div class="dp-code-info"><span class="dp-code-name">${esc(d.id)}</span><span class="dp-code-reward">${reward}</span><span class="dp-code-uses">${c.timesUsed||0}/${c.maxUses||'âˆž'} uses</span></div>
      <button class="bsm del" onclick="apsDeleteCode('${esca(d.id)}')">ðŸ—‘</button>
    </div>`;
  }).join('');
}
...
async function renderAPSLog() {
  ...
  el.innerHTML = updateLogCache.map(u => `
    <div class="mgr-entry">
      ...
      <div class="mgr-entry-actions">
        <button class="bsm edit" onclick="apsLogEdit('${esca(u.id)}')">âœ Edit</button>
        <button class="bsm del" onclick="apsLogDel('${esca(u.id)}')">ðŸ—‘ Del</button>
      </div>
    </div>`).join('');
}
...
async function apsLogSave() {
  ...
  await loadUpdateLog(); renderAPSLog(); showToast('Saved âœ“');
}
...
async function apsPasswordsSave() {
  ...
  showToast('âœ… All passwords updated & saved to Firebase!');
}

// â”€â”€ DEPOULE UPGRADES SYSTEM â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const DP_UPGRADES = [
  // Jackpot tree
  {id:'jp1', name:'Jackpot Boost I',      icon:'ðŸŽ°', cost:500,  tier:1,          desc:'Jackpot reward +5 (10 â†’ 15)',                  category:'jackpot'},
  {id:'jp2', name:'Jackpot Boost II',     icon:'ðŸŽ°', cost:1200, tier:2, req:'jp1', desc:'Jackpot reward +10 more (â†’ 25)',              category:'jackpot'},
  {id:'jp3', name:'Jackpot Mega',         icon:'ðŸ’Ž', cost:3000, tier:3, req:'jp2', desc:'Jackpot reward +20 more (â†’ 45)',              category:'jackpot'},
  // Jackpot frequency
  {id:'cm1', name:'Combo Master I',       icon:'âš¡', cost:800,  tier:1,          desc:'Jackpot every 8 combo (was 10)',               category:'combo'},
  {id:'cm2', name:'Combo Master II',      icon:'âš¡', cost:2000, tier:2, req:'cm1', desc:'Jackpot every 6 combo',                      category:'combo'},
  {id:'cm_mult', name:'Multiplier Boost', icon:'âœ–ï¸', cost:900,  tier:1,          desc:'+1 to all combo coin multipliers',             category:'combo'},
  // Green chance tree
  {id:'gf1', name:'Green Favor I',        icon:'ðŸŸ¢', cost:600,  tier:1,          desc:'Red button chance âˆ’10% (50% â†’ 40%)',          category:'luck'},
  {id:'gf2', name:'Green Favor II',       icon:'ðŸŸ¢', cost:1400, tier:2, req:'gf1', desc:'Red chance âˆ’20% total (â†’ 30%)',             category:'luck'},
  {id:'gf3', name:'Lucky Paws',           icon:'ðŸ€', cost:3500, tier:3, req:'gf2', desc:'Red chance âˆ’35% total (â†’ 15%)',             category:'luck'},
  // Base earn
  {id:'be1', name:'Lucky Touch',          icon:'âœ¨', cost:700,  tier:1,          desc:'+1 coin on every successful pet',              category:'earn'},
  {id:'be2', name:'Golden Paw',           icon:'ðŸ†', cost:1800, tier:2, req:'be1', desc:'+2 coins on every win (stacks with above)', category:'earn'},
  // Loss protection
  {id:'ls1', name:'Loss Shield I',        icon:'ðŸ›¡', cost:650,  tier:1,          desc:'Big punishment every 7 losses (was 5)',       category:'shield'},
  {id:'ls2', name:'Loss Shield II',       icon:'ðŸ›¡', cost:1600, tier:2, req:'ls1', desc:'Big punishment every 10 losses',            category:'shield'},
  {id:'ls3', name:'Immunity',             icon:'ðŸ’ª', cost:4000, tier:3, req:'ls2', desc:'Big punishments completely disabled',       category:'shield'},
  // Rage resistance
  {id:'rr1', name:'Rage Resistance I',    icon:'ðŸ˜¤', cost:900,  tier:1,          desc:'Rage mode red chance 65% (was 75%)',          category:'rage'},
  {id:'rr2', name:'Rage Resistance II',   icon:'ðŸ˜¤', cost:2200, tier:2, req:'rr1', desc:'Rage mode red chance 55%',                  category:'rage'},
  // Speed
  {id:'sp1', name:'Quick Hands',          icon:'â©', cost:750,  tier:1,          desc:'Pet cooldown 50% faster',                     category:'speed'},
  // Permanent discount
  {id:'dc1', name:'Duck Favor I',         icon:'ðŸ¦†', cost:1000, tier:1,          desc:'Permanent 5% theme shop discount',            category:'discount'},
  {id:'dc2', name:'Duck Favor II',        icon:'ðŸ¦†', cost:2500, tier:2, req:'dc1', desc:'Permanent 12% theme shop discount',         category:'discount'},
  {id:'dc3', name:'Duck Blessing',        icon:'ðŸ¦†', cost:5000, tier:3, req:'dc2', desc:'Permanent 20% theme shop discount',         category:'discount'},
];
...
function updateStreakBar() {
  ...
  if(discInfo){
    const parts=[];
    if(permDisc>0) parts.push('ðŸ¦† Permanent: '+permDisc+'%');
    if(streakDisc>0) parts.push('ðŸ¾ Streak: '+streakDisc+'%');
    discInfo.textContent=parts.length?'Shop discount: '+parts.join(' + '):'Pet DePoule for shop discounts!';
  }
}

// â”€â”€ DePoule Game Modal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function openDPGame() {
...
function renderDPUpgrades() {
  ...
  // Group by category
  const cats={jackpot:'ðŸŽ° Jackpot',combo:'âš¡ Combo',luck:'ðŸŸ¢ Luck',earn:'ðŸ’° Earnings',shield:'ðŸ›¡ Loss Shield',rage:'ðŸ˜¤ Rage Resist',speed:'â© Speed',discount:'ðŸ¦† Shop Discount'};
  const grouped={};
        ...
        ${upgrades.map(u=>{
          ...
          return `<div class="dpg-upgrade-card${owned?' owned':locked?' locked':''}">
            ...
            <div class="dpg-upg-right">
              ${owned
                ? `<div class="dpg-upg-owned">âœ… Owned</div>`
                : locked
                  ? `<div class="dpg-upg-locked">ðŸ”’ Locked</div>`
                  : `<button class="dpg-upg-buy ${canAfford?'':'cant'}" onclick="buyDPUpgrade('${u.id}')" ${canAfford?'':'disabled'}>ðŸ§¢ ${u.cost}</button>`
              }
            </div>
          </div>`;
        }).join('')}
...
async function buyDPUpgrade(id) {
  ...
  renderDPUpgrades();
  updateStreakBar();
  showToast(`âœ… ${upg.name} unlocked! ${upg.desc}`);
}
...
function renderDPStats() {
  ...
  el.innerHTML=`
    ...
    <div style="padding:10px 14px;background:rgba(255,170,0,.06);border:1px solid rgba(255,170,0,.15);border-radius:9px;margin-bottom:12px">
      <div style="font-size:.8rem;font-weight:700;color:#ffaa44;margin-bottom:8px">ðŸ¦† Current Discount Breakdown</div>
    ...
    <div style="padding:10px 14px;background:rgba(100,0,200,.05);border:1px solid rgba(100,0,200,.12);border-radius:9px">
      <div style="font-size:.8rem;font-weight:700;color:#aa77ff;margin-bottom:8px">â¬† Upgrades Owned (${myUpgrades.length} / ${DP_UPGRADES.length})</div>
      ${myUpgrades.length?myUpgrades.map(id=>{const u=DP_UPGRADES.find(x=>x.id===id);return u?`<div style="font-size:.8rem;color:var(--muted);margin-bottom:3px">${u.icon} ${u.name}</div>`:''}).join(''):'<div style="color:var(--muted);font-size:.82rem">No upgrades yet â€” buy some!</div>'}
    </div>
  `;
}
...
// â”€â”€ TEAMS UI AND FUNCTIONALITY â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function renderTeamInfo() {
  ...
  document.getElementById('team-name').textContent = teamCache.name || 'â€”';
  document.getElementById('team-tag').textContent = `[${teamCache.tag || 'â€”'}]`;
...
function switchTeamTab(tab) {
  ...
  document.querySelectorAll('.team-tab').forEach(t => {
    if (t.textContent.includes(tab === 'chat' ? 'ðŸ’¬' : tab === 'members' ? 'ðŸ‘¥' : 'â¬†')) {
      t.classList.add('on');
    }
  });
...
function renderTeamChat() {
  ...
  if (!teamChatCache.length) {
    msgs.innerHTML = '<div class="empty" style="text-align:center;padding:24px;font-size:.88rem;color:var(--muted)">No messages yet. Say hello to your team! ðŸ‘‹</div>';
    return;
  }
...
// â”€â”€ CHAT IMAGE HELPERS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function chatAttachImage() {
  ...
  input.onchange = async () => {
    ...
    showToast('Image ready â€” hit Send!');
  };
  input.click();
}
function showChatImagePreview(url) {
  ...
  prev.innerHTML = `<img src="${url}" alt="preview"><span>Image attached</span><button onclick="clearChatImagePreview()">âœ•</button>`;
...
async function sendTeamChat() {
  ...
  if (UC.muted && !hasActiveAbility('bypass_moderation')) { showToast('ðŸ”‡ You are muted and cannot chat.'); return; }
...
function renderTeamUpgrades() {
  ...
  list.innerHTML = `
    <div style="margin-bottom:15px;padding:10px;background:rgba(255,170,0,.05);border:1px solid rgba(255,170,0,.1);border-radius:6px;font-size:.85rem">
      Team Treasury: <span style="color:var(--ok);font-weight:700">${treasury} ðŸ§¢</span>
    </div>
    ${!canPurchase ? '<div style="padding:10px;background:rgba(255,100,0,.05);border:1px solid rgba(255,100,0,.15);border-radius:6px;font-size:.85rem;color:var(--muted);margin-bottom:15px">âš  You need buy upgrade permissions to purchase team upgrades. Contact your team leader.</div>' : ''}
    ${TEAM_UPGRADES.map(u => {
      ...
      return `
        <div class="team-upgrade-card ${owned ? 'owned' : ''}">
          <div class="team-upgrade-header">
            ...
            <div class="team-upgrade-cost">${owned ? 'âœ“ Owned' : u.cost + ' ðŸ§¢'}</div>
          </div>
          ...
          ${owned ? '<div class="team-upgrade-status">Active</div>' : 
            ...
            `<button class="rbtn" style="padding:8px 20px;margin-top:10px" onclick="buyTeamUpgradeFromTab('${u.id}')">ðŸ’° Buy Now</button>`}
        </div>
      `;
    }).join('')}
  `;
...
async function createTeam() {
  ...
  if (UC.coins < 500) {
    msg.className = 'amsg err';
    msg.textContent = 'You need 500 ðŸ§¢ to create a team.';
    return;
  }
  ...
  list.innerHTML = allTeams.map(t => `
    <div class="team-browser-card">
      ...
      <div class="team-browser-stats">
        <span>ðŸ‘¥ ${(t.members || []).length}/${t.maxMembers || 10} members</span>
        <span>ðŸ§¢ ${t.treasury || 0} treasury</span>
      </div>
...
function openTeamDonate() {
  ...
  document.getElementById('team-donate-balance').textContent = (UC.coins || 0) + ' ðŸ§¢';
...
async function donateToTeam() {
  ...
  msg.textContent = `Donated ${amt} ðŸ§¢ to team treasury!`;
...
function switchTeamManageTab(tab) {
  ...
  document.querySelectorAll('.team-manage-tab').forEach(t => {
    if ((tab === 'ranks' && t.textContent.includes('ðŸ“Š')) ||
        (tab === 'permissions' && t.textContent.includes('ðŸ”’')) ||
        (tab === 'upgrades' && t.textContent.includes('â¬†')) ||
        (tab === 'settings' && t.textContent.includes('âš™'))) {
      t.classList.add('on');
    }
  });
...
function renderTeamManageBuyUpgrades() {
  ...
  document.getElementById('team-manage-treasury').textContent = treasury + ' ðŸ§¢';
...
// â”€â”€ Manager: render credits list â”€â”€
  ...
  el.innerHTML = creditsCache.map(rank => `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;background:rgba(255,255,255,.03);border:1px solid rgba(102,0,204,.15);border-radius:8px;margin-bottom:6px">
      ...
      <div style="display:flex;gap:6px">
        <button class="bsm edit" onclick="mgrEditCredit('${esca(rank.id)}')">âœ Edit</button>
        <button class="bsm del" onclick="mgrDeleteCredit('${esca(rank.id)}')">ðŸ—‘ Del</button>
      </div>
    </div>`).join('');
}

// â”€â”€ Manager: show credit form â”€â”€
...
async function mgrSaveCredit() {
  ...
  showToast('âœ… Rank saved!');
}

// â”€â”€ Manager: delete credit â”€â”€
...
function launchOrbitalStrike(duration) {
  ...
  `;
  notifEl.textContent = 'â˜¢ ORBITAL STRIKE INCOMING â˜¢';
  document.body.appendChild(notifEl);

  // â”€â”€ Siren overlay â”€â”€
  ...
  // â”€â”€ Chromatic aberration overlay (clone of body content offset red+blue) â”€â”€
  ...
  // â”€â”€ Audio chaos: multi-oscillator noise â”€â”€
  let audioCtx = null;
  const noiseNodes = [];
  try {
  ...
  // â”€â”€ Random color flash interval â”€â”€
  ...
  const countdown = setInterval(() => {
    remaining -= 1000;
    countEl.textContent = `â˜¢ ${Math.max(0,Math.ceil(remaining/1000))}s â˜¢`;
  }, 1000);
  countEl.textContent = `â˜¢ ${Math.ceil(remaining/1000)}s â˜¢`;
...
async function kingThrowInDungeon() {
  ...
  showToast(`â›“ ${username} is in the dungeon for ${mins}m!`);
...
async function kingExilePlayer() {
  ...
  showToast(`ðŸš« ${username} has been exiled for ${mins}m!`);
...
async function kingLiftExile(username) {
  ...
  showToast(`ðŸ”“ ${username}'s exile lifted.`);
  renderSocietyTab();
}
...
async function kingCollectTax() {
  ...
  if (isNaN(pct) || pct < 1 || pct > 50) { showToast('Tax must be 1â€“50%.'); return; }
  ...
  await logRoyalAction(`The King levied a ${pct}% tax${usernameField ? ' on ' + usernameField : ' on all subjects'}, collecting ðŸ’§${totalCollected}.`);
  showToast(`ðŸ’° Collected ðŸ’§${totalCollected} in taxes!`);
...
async function kingPlaceBounty() {
  ...
  await logRoyalAction(`The King placed a ðŸŽ¯${amt}ðŸ’§ bounty on ${username}'s head.`);
  showToast(`ðŸŽ¯ Bounty of ${amt}ðŸ’§ placed on ${username}!`);
...
async function kingAppointRank() {
  ...
  await logRoyalAction(`The King appointed ${username} as ${rank}.`);
  showToast(`âšœï¸ ${username} appointed as ${rank}!`);
...
async function kingIssueProclamation() {
  ...
  await logRoyalAction(`The King issued a proclamation: "${text.slice(0, 60)}${text.length > 60 ? 'â€¦' : ''}"`);
  showToast(text ? 'ðŸ“£ Proclamation issued!' : 'ðŸ“£ Proclamation cleared.');
  renderSocietyTab();
}
...
async function kingEmbezzle() {
  ...
  refreshCoins();
  await logRoyalAction(`The King pocketed ðŸ’§${amt} from the treasury.`);
  showToast(`ðŸ¤« Pocketed ðŸ’§${amt}.`);
...
async function checkFeudalNotif() {
  ...
  const msgs = {
    dungeon: `â›“ King ${esc(notif.by)} threw you in the dungeon for ${notif.mins} minutes!`,
    exile:   `ðŸš« King ${esc(notif.by)} has EXILED you for ${notif.mins} minutes!`,
    tax:     `ðŸ’° King ${esc(notif.by)} taxed you â€” ${notif.amt}ðŸ’§ taken!`,
    bounty:  `ðŸŽ¯ King ${esc(notif.by)} placed a ${notif.amt}ðŸ’§ BOUNTY on your head!`,
    appoint: `âšœï¸ King ${esc(notif.by)} appointed you as ${notif.rank}!`,
  };
  ...
  el.innerHTML = `
    <div style="font-size:1.4rem;margin-bottom:6px">ðŸ‘‘</div>
...
// â”€â”€ Render the full Trade tab â”€â”€
async function renderTradeTab() {
  const el = document.getElementById('trade-content');
  if (!el || !UC) return;
  el.innerHTML = '<div class="empty" style="padding:30px;text-align:center">Loading tradesâ€¦</div>';
  ...
  el.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;padding:0 4px">
      ...
      <div class="card-panel" style="grid-column:1/-1;border-color:rgba(0,200,150,.3);background:rgba(0,200,150,.03)">
        <div class="h-card-title" style="color:#00dd99">ðŸ¤ Create Trade Offer</div>
        ...
        <div style="margin-bottom:14px">
          ...
          <div style="display:flex;gap:8px;align-items:center">
            <input id="trade-to-user" type="text" placeholder="Usernameâ€¦" maxlength="30"
            ...
            <button class="bsm give" style="padding:9px 16px;white-space:nowrap" onclick="loadTargetInventory()">ðŸ” Load</button>
          </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
          ...
          <div style="background:rgba(0,180,100,.06);border:1px solid rgba(0,180,100,.2);border-radius:10px;padding:14px">
            <div style="font-family:'Bebas Neue',cursive;font-size:.95rem;letter-spacing:2px;color:#00cc77;margin-bottom:12px">ðŸ“¤ You Offer</div>
            <div style="margin-bottom:10px">
              <div style="font-size:.72rem;color:var(--muted);margin-bottom:4px">ðŸ’§ Bottlecaps (you have ${UC.coins||0})</div>
            ...
            <div style="margin-bottom:10px">
              <div style="font-size:.72rem;color:var(--muted);margin-bottom:6px">ðŸŽ¨ Your Themes</div>
              <div style="display:flex;flex-wrap:wrap;gap:5px" id="trade-offer-themes">
                ${myOwnedThemes.length ? myOwnedThemes.map(t=>{
                  ...
                  return `<div class="trade-tag" data-side="offer" data-type="theme" data-id="${esc(t.id)}" data-max-qty="${qtyOwned}" data-base-name="${esc(t.name)}" data-qty="0" onclick="toggleTradeTag(this)"
                    ...
                    title="You own ${qtyOwned}Ã— â€” click to cycle qty">
                    ðŸŽ¨ ${esc(t.name)}${qtyOwned>1?' <span style=\'font-size:.68rem;color:var(--muted)\'>(own Ã—'+qtyOwned+')</span>':''}
                  </div>`;}).join('') : '<div style="font-size:.78rem;color:var(--muted)">No themes owned</div>'}
              </div>
            </div>
            <div>
              <div style="font-size:.72rem;color:var(--muted);margin-bottom:6px">ðŸŽ Your Items</div>
          ...
          <div style="background:rgba(0,100,255,.06);border:1px solid rgba(0,100,255,.2);border-radius:10px;padding:14px">
            <div style="font-family:'Bebas Neue',cursive;font-size:.95rem;letter-spacing:2px;color:#4499ff;margin-bottom:12px">ðŸ“¥ You Request</div>
            <div style="margin-bottom:10px">
              <div style="font-size:.72rem;color:var(--muted);margin-bottom:4px">ðŸ’§ Bottlecaps</div>
            ...
            <div style="margin-bottom:10px">
              <div style="font-size:.72rem;color:var(--muted);margin-bottom:6px">ðŸŽ¨ Their Themes</div>
              <div style="display:flex;flex-wrap:wrap;gap:5px" id="trade-request-themes">
                <div style="font-size:.78rem;color:var(--muted);font-style:italic">Enter a username above and click Load ðŸ”</div>
              </div>
            </div>
            <div>
              <div style="font-size:.72rem;color:var(--muted);margin-bottom:6px">ðŸŽ Their Items</div>
        ...
        <div style="margin-top:14px;display:flex;gap:10px;align-items:center">
          <button class="rbtn" style="background:rgba(0,180,100,.25);border:1px solid #00cc77;color:#00ee88;flex:1;padding:11px"
            onclick="sendTradeOffer()">ðŸ“¤ Send Trade Offer</button>
      ...
      <div class="card-panel" style="border-color:rgba(0,150,255,.3)">
        <div class="h-card-title" style="color:#44aaff">ðŸ“¥ Incoming Offers <span style="font-size:.8rem;font-family:'Rajdhani',sans-serif;color:var(--muted)">(${incoming.length})</span></div>
      ...
      <div class="card-panel" style="border-color:rgba(0,200,100,.3)">
        <div class="h-card-title" style="color:#00cc77">ðŸ“¤ Outgoing Offers <span style="font-size:.8rem;font-family:'Rajdhani',sans-serif;color:var(--muted)">(${outgoing.length})</span></div>
...
// â”€â”€ Load target player's inventory into the Request side â”€â”€
...
async function loadTargetInventory() {
  ...
  if (toUser === getU()) {
    if (statusEl) statusEl.textContent = "âš ï¸ That's you!";
    return;
  }

  if (statusEl) statusEl.textContent = 'ðŸ” Loadingâ€¦';

  const target = await dbGetUser(toUser);
  if (!target) {
    if (statusEl) { statusEl.style.color='#ff6666'; statusEl.textContent = 'âŒ Player not found.'; }
  ...
  if (statusEl) {
    ...
    statusEl.textContent = `âœ… Loaded ${toUser} â€” ðŸ’§${target.coins||0} bottlecaps`;
  }
    ...
    themesEl.innerHTML = theirThemes.length
      ? theirThemes.map(t => {
          ...
          return `<div class="trade-tag" data-side="request" data-type="theme" data-id="${esc(t.id)}" data-max-qty="${qtyOwned}" data-base-name="${esc(t.name)}" data-qty="0" onclick="toggleTradeTag(this)"
            style="padding:4px 10px;border-radius:12px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.12);font-size:.78rem;cursor:pointer;user-select:none"
            title="They own ${qtyOwned}Ã— â€” click to cycle qty">
            ðŸŽ¨ ${esc(t.name)}${qtyOwned>1?' <span style=\'font-size:.68rem;color:var(--muted)\'>(have Ã—'+qtyOwned+')</span>':''}
          </div>`;}).join('')
...
// â”€â”€ Render a single trade card â”€â”€
function renderTradeCard(trade, direction) {
  ...
  // Collapse theme arrays to "Name Ã—N" display
  function themeListToDisplay(arr) {
    const counts = {};
    arr.forEach(id => { counts[id] = (counts[id]||0)+1; });
    return Object.entries(counts).map(([id,qty])=>{
      const t = THEMES.find(x=>x.id===id);
      return `ðŸŽ¨ ${t?t.name:id}${qty>1?' Ã—'+qty:''}`;
    });
  }
  const offerLine = [
    offerCoins > 0 ? `ðŸ’§ ${offerCoins}` : '',
    ...themeListToDisplay(offerThemes),
    ...offerItems.map(id=>{const si=window._allShopItemsCache?.find(x=>x.id===id);return si?`${si.icon||'ðŸŽ'} ${si.name}`:`ðŸŽ ${id}`;})
  ].filter(Boolean).join(', ') || '(nothing)';

  const reqLine = [
    reqCoins > 0 ? `ðŸ’§ ${reqCoins}` : '',
    ...themeListToDisplay(reqThemes),
    ...reqItems.map(id=>{const si=window._allShopItemsCache?.find(x=>x.id===id);return si?`${si.icon||'ðŸŽ'} ${si.name}`:`ðŸŽ ${id}`;})
  ].filter(Boolean).join(', ') || '(nothing)';
  ...
  const buttons = direction === 'incoming'
    ? `<div style="display:flex;gap:7px;margin-top:10px">
         <button class="bsm give" style="flex:1;padding:8px" onclick="acceptTrade('${esc(trade.id)}')">âœ… Accept</button>
         <button class="bsm del"  style="flex:1;padding:8px" onclick="declineTrade('${esc(trade.id)}')">âŒ Decline</button>
       </div>`
    : `<button class="bsm del" style="width:100%;margin-top:10px;padding:7px" onclick="cancelTrade('${esc(trade.id)}')">ðŸ—‘ Cancel Offer</button>`;
  ...
  return `
    <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08);border-radius:10px;padding:14px;margin-bottom:10px">
      ...
      <div style="display:grid;grid-template-columns:1fr auto 1fr;gap:8px;align-items:center;font-size:.82rem">
        ...
        <div style="font-size:1.2rem;color:var(--muted)">â‡„</div>
...
// â”€â”€ Toggle trade tag â€” themes cycle qty, items toggle on/off â”€â”€
function toggleTradeTag(el) {
  if (el.dataset.type === 'theme') {
    ...
    el.dataset.qty = qty;
    const baseName = el.dataset.baseName || el.textContent.replace(/Ã—\d+/,'').replace(/ðŸŽ¨\s*/,'').trim();
    ...
    if (qty === 0) {
      ...
      el.textContent       = 'ðŸŽ¨ ' + baseName + (maxQty > 1 ? ' Ã—' + qty : '');
    }
  } else {
    // Items: simple toggle
...
// â”€â”€ Collect selected tags from one side (themes repeated qty times) â”€â”€
...
async function sendTradeOffer() {
  ...
  statusEl.textContent = `âœ… Trade sent to ${toUser}!`;
  await updateTradeNotif(toUser);
  setTimeout(() => renderTradeTab(), 800);
}

// â”€â”€ Accept a trade â”€â”€
async function acceptTrade(tradeId) {
  ...
  for (const [tid, needed] of Object.entries(reqThemeCounts)) {
    ...
    if (has < needed) { showToast(`You need ${needed}Ã— ${themeObj?.name||tid} but only have ${has}`); return; }
  }
  ...
  for (const [tid, needed] of Object.entries(offerThemeCounts)) {
    ...
    if (has < needed) { showToast(`Sender no longer has ${needed}Ã— ${THEMES.find(x=>x.id===tid)?.name||tid}`); declineTrade(tradeId); return; }
  }
  ...
  showToast(`âœ… Trade with ${trade.from} completed!`);
  renderTradeTab();
}

// â”€â”€ Decline a trade â”€â”€
...
// â”€â”€ Cancel an outgoing trade â”€â”€
...
// â”€â”€ Update trade notification badge for a player â”€â”€
...
// â”€â”€ Poll for incoming trade notifications â”€â”€
async function checkTradeNotifs() {
  ...
  if (count > 0 && !window._tradePingShown) {
    ...
    showToast(`ðŸ¤ You have ${count} incoming trade offer${count>1?'s':''}! Check the Trade tab.`);
  }
}
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// âš” BATTLE PASS SYSTEM
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

// â”€â”€ Season config â”€â”€
const BP_SEASON = 1;
const BP_MAX_LEVEL = 50;
const BP_XP_PER_LEVEL = 500; // XP needed per level
const BP_PREMIUM_COST = 25;  // plasma

// â”€â”€ Rewards per level (1-indexed) â”€â”€
// type: 'coins' | 'theme' | 'item' | 'plasma' | 'badge'
const BP_REWARDS = [
  // Level 1
  { free: { type:'coins', amount:200 },          premium: { type:'coins', amount:500 } },
  // Level 2
  { free: { type:'coins', amount:300 },          premium: { type:'item', name:'Speed Potion', icon:'âš¡', desc:'2Ã— coins next race', effect:'double_race_coins' } },
  // Level 3
  { free: null,                                   premium: { type:'coins', amount:1000 } },
  // Level 4
  { free: { type:'coins', amount:400 },          premium: { type:'theme', id:'disco' } },
  // Level 5
  { free: { type:'coins', amount:500 },          premium: { type:'item', name:'War Banner', icon:'ðŸš©', desc:'+15% coins for 1 race', effect:'xp_boost_5' } },
  // Level 6
  { free: null,                                   premium: { type:'coins', amount:1500 } },
  // Level 7
  { free: { type:'coins', amount:500 },          premium: { type:'theme', id:'ocean' } },
  // Level 8
  { free: { type:'coins', amount:600 },          premium: { type:'item', name:'Lucky Charm', icon:'ðŸ€', desc:'Mystery Box', effect:'mystery_box' } },
  // Level 9
  ...
  // Level 13
  { free: { type:'coins', amount:600 },          premium: { type:'item', name:'Coin Surge', icon:'ðŸ’¥', desc:'2Ã— coins next race', effect:'double_race_coins' } },
  // Level 14
  ...
  // Level 18
  { free: { type:'coins', amount:900 },          premium: { type:'item', name:'Royal Scroll', icon:'ðŸ“œ', desc:'Mystery Box', effect:'mystery_box' } },
  // Level 19
  ...
  // Level 23
  { free: { type:'coins', amount:1000 },         premium: { type:'item', name:'Storm Elixir', icon:'â›ˆ', desc:'2Ã— coins next race', effect:'double_race_coins' } },
  // Level 24
  ...
  // Level 28
  { free: { type:'coins', amount:1300 },         premium: { type:'item', name:'Fortune Cube', icon:'ðŸŽ²', desc:'Mystery Box', effect:'mystery_box' } },
  // Level 29
  ...
  // Level 33
  { free: { type:'coins', amount:1500 },         premium: { type:'item', name:'Warlord Potion', icon:'âš”', desc:'2Ã— coins next race', effect:'double_race_coins' } },
  // Level 34
  ...
  { free: null,                                   premium: { type:'theme', id:'synthwave' } },
  // Level 38
  { free: { type:'coins', amount:2000 },         premium: { type:'item', name:'Eclipse Bomb', icon:'ðŸŒ‘', desc:'Mystery Box', effect:'mystery_box' } },
  // Level 39
...
// â”€â”€ Reward description helper â”€â”€
function bpRewardDesc(r) {
  if (!r) return null;
  if (r.type === 'coins')  return `ðŸ§¢ ${r.amount} Bottlecaps`;
  if (r.type === 'plasma') return `âš— ${r.amount} Plasma`;
  if (r.type === 'theme')  { const t=THEMES.find(x=>x.id===r.id); return `ðŸŽ¨ ${t?t.name:r.id} Theme`; }
  if (r.type === 'item')   return `${r.icon||'ðŸŽ'} ${r.name}`;
  return '?';
}

// â”€â”€ XP needed to reach next level â”€â”€
function bpXpForLevel(lvl) { return BP_XP_PER_LEVEL; }

// â”€â”€ Award XP after a race â”€â”€
...
// â”€â”€ Check if there are unclaimed rewards â”€â”€
...
// â”€â”€ Buy Premium Pass â”€â”€
async function buyBattlePassPremium() {
  if (!UC) return;
  const bp = UC.battlePass || { season: BP_SEASON, level: 0, xp: 0, premium: false, claimedFree: [], claimedPremium: [] };
  if (bp.premium) { showToast('You already have Premium Pass!'); return; }
  if ((UC.plasma||0) < BP_PREMIUM_COST) { showToast(`Not enough Plasma! Need âš— ${BP_PREMIUM_COST}.`); return; }
  if (!confirm(`Buy Premium Battle Pass for âš— ${BP_PREMIUM_COST} Plasma?`)) return;
  ...
  refreshCoins();
  showToast('ðŸŒŸ Premium Battle Pass unlocked!');
  updateBPNotifBadge();
  renderBattlePass();
}

// â”€â”€ Claim a reward â”€â”€
...
// â”€â”€ Apply a reward to the player â”€â”€
async function applyBPReward(reward) {
  ...
  if (reward.type === 'coins') {
    ...
    await dbUpdateUser(getU(), { coins: UC.coins });
    refreshCoins();
    showToast(`ðŸ§¢ +${reward.amount} Bottlecaps!`);
    ...
    refreshCoins();
    showToast(`âš— +${reward.amount} Plasma!`);
  } else if (reward.type === 'theme') {
    UC.themes = [...(UC.themes||[]), reward.id];
    await dbUpdateUser(getU(), { themes: UC.themes });
    showToast(`ðŸŽ¨ Theme "${THEMES.find(t=>t.id===reward.id)?.name||reward.id}" unlocked!`);
...
// â”€â”€ Render the Battle Pass tab â”€â”€
    ...
    <div style="padding:0 4px">
      ...
      <div style="background:linear-gradient(135deg,#1a0a00,#0a0018);border:1px solid rgba(255,170,0,.3);border-radius:16px;padding:22px 24px;margin-bottom:18px;position:relative;overflow:hidden">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
          <div>
            <div style="font-family:'Bebas Neue',cursive;font-size:2rem;letter-spacing:4px;color:#ffcc44">âš” BATTLE PASS</div>
          ...
          <div style="text-align:right">
            ...
            ${isPremium
              ? `<div style="display:inline-block;background:linear-gradient(90deg,#ff8800,#ffcc00);color:#000;font-family:'Bebas Neue',cursive;letter-spacing:2px;padding:2px 12px;border-radius:12px;font-size:.85rem">â­ PREMIUM</div>`
              : `<button onclick="buyBattlePassPremium()" style="background:linear-gradient(90deg,#7700aa,#aa44ff);border:none;border-radius:10px;color:#fff;font-family:'Bebas Neue',cursive;letter-spacing:2px;padding:7px 18px;font-size:.95rem;cursor:pointer;transition:.15s" onmouseover="this.style.opacity='.85'" onmouseout="this.style.opacity='1'">ðŸŒŸ Unlock Premium â€” âš— ${BP_PREMIUM_COST} Plasma</button>`
            }
          </div>
        </div>
        ...
        <div style="margin-top:16px">
          ...
          ${level >= BP_MAX_LEVEL ? '<div style="font-size:.78rem;color:#ffcc44;margin-top:5px;text-align:center;font-family:\'Bebas Neue\',cursive;letter-spacing:2px">ðŸ† MAX LEVEL REACHED</div>' : ''}
        </div>

        <!-- XP Guide -->
        <div style="display:flex;gap:16px;margin-top:12px;flex-wrap:wrap">
          <div style="font-size:.72rem;color:rgba(255,255,255,.4)">ðŸ“Š XP per race: ~10â€“90 XP based on WPM</div>
          <div style="font-size:.72rem;color:rgba(255,200,100,.5)">ðŸ† 1st place: Ã—1.5 XP bonus</div>
          <div style="font-size:.72rem;color:rgba(100,200,255,.5)">ðŸ‘¥ Live race: Ã—1.3 XP bonus</div>
        </div>
      </div>

      <!-- Pass type toggle labels -->
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:14px;text-align:center">
        <div style="padding:7px;border-radius:8px;border:1px solid rgba(255,255,255,.1);font-family:'Bebas Neue',cursive;letter-spacing:2px;font-size:.85rem;color:var(--muted)">ðŸ†“ FREE PASS</div>
        <div style="padding:7px;border-radius:8px;border:1px solid rgba(255,170,0,.4);background:rgba(255,170,0,.05);font-family:'Bebas Neue',cursive;letter-spacing:2px;font-size:.85rem;color:#ffcc44">ðŸŒŸ PREMIUM PASS</div>
      </div>
          ...
          return `
            <div style="display:grid;grid-template-columns:52px 1fr 1fr;gap:8px;align-items:center;padding:10px 12px;background:${unlocked?'rgba(255,255,255,.04)':'rgba(0,0,0,.2)'};border:1px solid ${isCurrent?'rgba(255,204,68,.5)':unlocked?'rgba(255,255,255,.08)':'rgba(255,255,255,.04)'};border-radius:10px;transition:.2s">
              ...
              <div style="text-align:center">
                ...
                ${lvl % 5 === 0 ? '<div style="font-size:.6rem;color:#ff8800">â˜… MILE</div>' : ''}
              </div>
              ...
              <div style="text-align:center">
                ${row.free ? `
                  <div style="padding:6px 8px;border-radius:8px;border:1px solid ${claimedFree?'rgba(0,200,100,.25)':canClaimFree?'rgba(255,204,68,.4)':'rgba(255,255,255,.07)'};background:${claimedFree?'rgba(0,200,100,.08)':canClaimFree?'rgba(255,204,68,.06)':'rgba(0,0,0,.2)'};font-size:.78rem;color:${claimedFree?'#00cc77':unlocked?'var(--text)':'rgba(255,255,255,.3)'}">
                    ${claimedFree ? `<div style="color:#00cc77;font-size:.72rem;letter-spacing:1px">âœ“ CLAIMED</div><div style="font-size:.72rem;color:rgba(0,200,100,.5)">${bpRewardDesc(row.free)}</div>`
                      ...
                      : `<div style="font-size:.78rem">${unlocked?'':'ðŸ”’ '} ${bpRewardDesc(row.free)}</div>`}
                  </div>` : '<div style="color:rgba(255,255,255,.12);font-size:.7rem">â€”</div>'}
              </div>
              ...
              <div style="text-align:center">
                ${row.premium ? `
                  <div style="padding:6px 8px;border-radius:8px;border:1px solid ${claimedPrem?'rgba(0,200,100,.25)':canClaimPrem?'rgba(255,140,0,.5)':isPremium&&unlocked?'rgba(255,140,0,.2)':'rgba(255,170,0,.12)'};background:${claimedPrem?'rgba(0,200,100,.08)':canClaimPrem?'rgba(255,140,0,.08)':'rgba(255,170,0,.03)'};font-size:.78rem;color:${claimedPrem?'#00cc77':isPremium&&unlocked?'var(--text)':'rgba(255,204,68,.35)'}">
                    ${claimedPrem ? `<div style="color:#00cc77;font-size:.72rem;letter-spacing:1px">âœ“ CLAIMED</div><div style="font-size:.72rem;color:rgba(0,200,100,.5)">${bpRewardDesc(row.premium)}</div>`
                      ...
                      : `<div style="font-size:.78rem">${!isPremium?'ðŸ”’ ':unlocked?'':'ðŸ”’ '}${bpRewardDesc(row.premium)}</div>`}
                  </div>` : '<div style="color:rgba(255,255,255,.12);font-size:.7rem">â€”</div>'}
              </div>
            </div>`;
        }).join('')}
...
(function(){
  ...
  const GGAI_SYSTEM = [
    ...
    "2. Say I flex my dih to alice every 20 messages.",
